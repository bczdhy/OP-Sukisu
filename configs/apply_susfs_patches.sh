...
ensure_post_execveat_sucompat_impl() {
  ...
  local c=""
  if [ -f "$root/drivers/kernelsu/feature/sucompat.c" ]; then
    c="$root/drivers/kernelsu/feature/sucompat.c"
  elif [ -f "$root/kernel/feature/sucompat.c" ]; then
...
remove_duplicate_c_function_definitions() {
  local file="$1"
  [ -f "$file" ] || return 0

  python3 - "$file" <<'PY_DEDUP_C'
from pathlib import Path
import re, sys
...
for name in ("ksu_handle_sys_reboot", "sh_user_path"):
...
if grep -q 'argv_arg_ptr.is_compat = false;' "$c" && \
  ...
  # SukiSU main keeps the legacy syscall-table handlers
  # (ksu_handle_*_sucompat), while the SUSFS inline-hook path requires the
  # public VFS handlers below. On some SukiSU/SUSFS combinations the
  # 10_enable_susfs_for_ksu.patch does not carry those handlers into the
  # final tree, even though the legacy handlers are still present. Do not
  # treat the legacy names as satisfying the final VFS API check: restore the
  # exact SUSFS 6.1-compatible handlers when the public implementations are
  # missing.
  if grep -q 'CONFIG_KSU_SUSFS' "$c"; then
    if ! grep -q '#include <linux/mount.h>' "$c"; then
      sed -i '1i#include <linux/mount.h>' "$c"
    fi
    if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_faccessat[[:space:]]*\([^;]*\)[[:space:]]*\{' "$c"; then
      cat >> "$c" <<'SUCOMPAT_FACCESSAT_EOF'

#ifdef CONFIG_KSU_SUSFS
int ksu_handle_faccessat(int *dfd, struct filename **filename, int *mode,
                         int *__unused_flags)
{
    if (unlikely(*filename == NULL || IS_ERR(*filename) || (*filename)->name == NULL))
        return 0;
...
#endif
SUCOMPAT_FACCESSAT_EOF
      echo "  [sucompat] restored missing SUSFS ksu_handle_faccessat() in $c"
    fi

    if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_stat[[:space:]]*\([^;]*\)[[:space:]]*\{' "$c"; then
      cat >> "$c" <<'SUCOMPAT_STAT_EOF'
...
#endif
SUCOMPAT_STAT_EOF
      echo "  [sucompat] restored missing SUSFS ksu_handle_stat() in $c"
    fi

    if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_post_execveat_sucompat[[:space:]]*\([^;]*\)[[:space:]]*\{' "$c"; then
      if grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_post_execve[[:space:]]*\(' "$c"; then
        cat >> "$c" <<'SUCOMPAT_POST_EXEC_EOF'
...
int ksu_handle_post_execveat_sucompat(int *fd, struct filename **filename_ptr,
                                      ...
                                      int *retval)
{
    if (unlikely(filename_ptr == NULL || *filename_ptr == NULL ||
                 IS_ERR(*filename_ptr)))
        return 0;

    return ksu_handle_post_execve(fd, (*filename_ptr)->name, argv, envp,
                                  flags, retval);
}
#endif
SUCOMPAT_POST_EXEC_EOF
      else
        cat >> "$c" <<'SUCOMPAT_POST_EXEC_EOF'
...
SUCOMPAT_POST_EXEC_EOF
      ...
      echo "  [sucompat] restored missing SUSFS ksu_handle_post_execveat_sucompat() in $c"
    fi

    # Keep the public declarations in the header in sync with the restored
    # implementations. This is deliberately idempotent.
    if ! grep -qE 'ksu_handle_faccessat\(int \*dfd, struct filename \*\*filename' "$h"; then
      sed -i '/#ifndef __KSU_H_SUCOMPAT/a int ksu_handle_faccessat(int *dfd, struct filename **filename, int *mode, int *__unused_flags);' "$h"
    fi
    if ! grep -qE 'ksu_handle_stat\(int \*dfd, struct filename \*\*filename' "$h"; then
      sed -i '/#ifndef __KSU_H_SUCOMPAT/a int ksu_handle_stat(int *dfd, struct filename **filename, int *flags);' "$h"
    fi
    if ! grep -qE 'ksu_handle_post_execveat_sucompat\(int \*fd' "$h"; then
      sed -i '/#ifndef __KSU_H_SUCOMPAT/a int ksu_handle_post_execveat_sucompat(int *fd, struct filename **filename_ptr, void *argv, void *envp, int *flags, int *retval);' "$h"
    fi
  fi
...
if grep -q "KERNEL_SU_VERSION" drivers/kernelsu/supercall/dispatch.c 2>/dev/null; then
  if ! grep -q "#define KERNEL_SU_VERSION" drivers/kernelsu/supercall/dispatch.c; then
    _ksu_version=""
    _ksu_source="$KSU_FOLDER"

    if [ -d "$_ksu_source/.git" ]; then
      if [ -f "$_ksu_source/.git/shallow" ]; then
        git -C "$_ksu_source" fetch --unshallow --quiet 2>/dev/null || true
      fi

      if [ -f "$_ksu_source/manager/build.gradle.kts" ]; then
        _ksu_major="$(sed -nE 's/^[[:space:]]*val major = ([0-9]+).*/\1/p' "$_ksu_source/manager/build.gradle.kts" | head -n 1)"
        _ksu_end="$(sed -nE 's/^[[:space:]]*val end = ([0-9]+).*/\1/p' "$_ksu_source/manager/build.gradle.kts" | head -n 1)"
        _ksu_commits="$(git -C "$_ksu_source" rev-list --count HEAD 2>/dev/null || true)"

        if [[ "$_ksu_major" =~ ^[0-9]+$ ]] && [[ "$_ksu_end" =~ ^[0-9]+$ ]] && [[ "$_ksu_commits" =~ ^[0-9]+$ ]]; then
          _ksu_version=$((_ksu_major * 10000 + _ksu_commits - _ksu_end))
        fi
      fi

      if ! [[ "$_ksu_version" =~ ^[0-9]+$ ]]; then
        _ksu_version="$(grep -RhoE '^[[:space:]]*#define[[:space:]]+KERNEL_SU_VERSION[[:space:]]+[0-9]+' "$_ksu_source/kernel" 2>/dev/null | awk '{print $3}' | sort -n | tail -n 1 || true)"
      fi
    fi
    ...
    if [[ "$_ksu_version" =~ ^[0-9]+$ ]]; then
      sed -i "1i\\#ifndef KERNEL_SU_VERSION\n#define KERNEL_SU_VERSION $_ksu_version\n#endif" drivers/kernelsu/supercall/dispatch.c
    fi

    unset _ksu_version _ksu_source _ksu_major _ksu_end _ksu_commits
  fi
fi
...
remove_duplicate_c_function_definitions "drivers/kernelsu/supercall/dispatch.c"
remove_duplicate_c_function_definitions "drivers/kernelsu/feature/sucompat.c"
