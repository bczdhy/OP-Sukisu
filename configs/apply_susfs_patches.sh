...
p = Path(sys.argv[1])
s = p.read_text()
old_call = r'(?m)^\s*ksu_handle_execveat_ksud\(\s*path\s*,\s*&argv\s*\)\s*;\s*$'
...
pat = r'(\b(?:static\s+)?void\s+ksu_handle_vfs_fstat\s*\(\s*int\s+fd\s*,\s*loff_t\s*\*\s*kstat_size_ptr\s*\)\s*\{)'
m = re.search(pat, s)
if not m:
    raise SystemExit(f'ksu_handle_vfs_fstat() not found in {p}')

body_start = m.end()
next_chunk = s[body_start:body_start + 300]
...
PYFSTAT
    ...
    if ! grep -qE 'ksu_handle_vfs_fstat[[:space:]]*\(' "$ksud_integration_c" || \
       ! awk '/ksu_handle_vfs_fstat[[:space:]]*\(/,/^\}/ { if ($0 ~ /loff_t orig_size = \*kstat_size_ptr;/) a=1; if ($0 ~ /size_t extra = 0;/) b=1 } END { exit !(a && b) }' "$ksud_integration_c"; then
      echo "::error::ksu_handle_vfs_fstat locals were not restored in $ksud_integration_c"
      grep -n -A12 'ksu_handle_vfs_fstat' "$ksud_integration_c" || true
      exit 1
    fi

    if grep -q 'ksu_handle_execveat_ksud(path, *\\&argv);' "$ksud_integration_c"; then
      echo "::error::Legacy 2-argument ksu_handle_execveat_ksud call remains in $ksud_integration_c"
      grep -n 'ksu_handle_execveat_ksud' "$ksud_integration_c" || true
      exit 1
    fi
