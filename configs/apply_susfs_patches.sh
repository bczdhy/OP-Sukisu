...
# The SUSFS SELinux-hide code uses the clean-policy snapshot symbol
# `backup_sepolicy`. Newer SUSFS expects it to be an externally visible
# KernelSU symbol. Some SukiSU trees used by OP13R do not provide the
# declaration/definition, which causes selinux_hide.c to fail at compile time.
#
# Keep this outside apply_sukisu_selinux_hide_fix.sh: it is a source-tree
# compatibility fix, not a change to the SELinux-hide patcher itself.
ensure_backup_sepolicy_symbol() {
  local root="$1"
  local cfile=""
  local hfile=""

  if [ -f "$root/kernel/feature/selinux_hide.c" ]; then
    cfile="$root/kernel/feature/selinux_hide.c"
    hfile="$root/kernel/include/ksu.h"
  elif [ -f "$root/feature/selinux_hide.c" ]; then
    cfile="$root/feature/selinux_hide.c"
    hfile="$root/include/ksu.h"
  elif [ -f "$root/drivers/kernelsu/feature/selinux_hide.c" ]; then
    cfile="$root/drivers/kernelsu/feature/selinux_hide.c"
    hfile="$root/drivers/kernelsu/include/ksu.h"
  fi

  [ -n "$cfile" ] || return 0

  echo "Ensuring SukiSU/SUSFS backup_sepolicy symbol: $cfile"

  # Do NOT define backup_sepolicy here. SukiSU already provides the single
  # global definition in selinux/rules.c. This compatibility step only makes
  # the existing symbol visible to selinux_hide.c through ksu.h.
  echo "  ℹ️ backup_sepolicy definition is provided by selinux/rules.c"

  # First expose the symbol through the normal KernelSU header when that
  # header is available. Some SukiSU trees do not include that header from
  # selinux_hide.c, so also make the declaration local to the implementation.
  if [ -f "$hfile" ]; then
    if ! grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*backup_sepolicy[[:space:]]*;' "$hfile"; then
      ensure_include_after_or_top "$hfile" \
        'extern struct selinux_policy *backup_sepolicy;'
      echo "  ✅ Added backup_sepolicy extern to $(basename "$hfile")"
    else
      echo "  ✅ backup_sepolicy extern already present"
    fi
  fi

  if ! grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*backup_sepolicy[[:space:]]*;' "$cfile"; then
    # Put the declaration before the first real function/type use. A local
    # extern is harmless when ksu.h also declares the same symbol and avoids
    # relying on a particular SukiSU include layout.
    sed -i '1i\extern struct selinux_policy *backup_sepolicy;' "$cfile"
    echo "  ✅ Added local backup_sepolicy extern to $(basename "$cfile")"
  else
    echo "  ✅ backup_sepolicy extern already visible in $(basename "$cfile")"
  fi
}
...
if grep -q 'void ksu_sucompat_init' "$h"; then
  ...
  # Keep compatibility prototypes inside the header guard.  Never append
  # declarations after the final #endif: doing so can produce '#endiflong ...'
  # and break the next compiler pass.
  if grep -qE '^[[:space:]]*long[[:space:]]+ksu_handle_faccessat_sucompat[[:space:]]*\(' "$c" ||      grep -qE '^[[:space:]]*long[[:space:]]+ksu_handle_stat_sucompat[[:space:]]*\(' "$c"; then
    python3 - "$h" "$c" <<'PY2'
from pathlib import Path
import re, sys
...
protos = []
if re.search(r'^[\s]*long\s+ksu_handle_faccessat_sucompat\s*\(', src, re.M):
    protos.append('long ksu_handle_faccessat_sucompat(int orig_nr, struct pt_regs *regs);')
if re.search(r'^[\s]*long\s+ksu_handle_stat_sucompat\s*\(', src, re.M):
    protos.append('long ksu_handle_stat_sucompat(int orig_nr, struct pt_regs *regs);')

# Remove these declarations wherever a previous run left them, including
# malformed '#endiflong ...' forms.
for proto in protos:
    text = text.replace(proto, '')
    text = text.replace('#endif' + proto, '#endif')

lines = text.splitlines()

# Find the header-guard terminator and insert before it.
idx = None
...
if idx is None:
    raise SystemExit('sucompat.h has no #endif terminator')
...
h = Path(sys.argv[1])
text = h.read_text()
proto = 'int ksu_handle_faccessat(int *dfd, const char __user **filename_user, int *mode, int *__unused_flags);'
lines = [ln for ln in text.splitlines() if ln.strip() != proto]
idx = next((i for i in range(len(lines)-1, -1, -1) if lines[i].strip() == '#endif'), None)
if idx is None:
    raise SystemExit('sucompat.h has no #endif terminator')
lines.insert(idx, proto)
h.write_text('\n'.join(lines).rstrip() + '\n')
...
text = h.read_text()
src = c.read_text()
if re.search(r'ksu_handle_stat\s*\(\s*int\s+\*dfd,\s*struct filename\s*\*\*', src):
    proto = 'int ksu_handle_stat(int *dfd, struct filename **filename, int *flags);'
else:
    proto = 'int ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags);'
lines = [ln for ln in text.splitlines() if ln.strip() not in (proto, 'int ksu_handle_stat(int *dfd, struct filename **filename, int *flags);', 'int ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags);')]
idx = next((i for i in range(len(lines)-1, -1, -1) if lines[i].strip() == '#endif'), None)
if idx is None:
    raise SystemExit('sucompat.h has no #endif terminator')
lines.insert(idx, proto)
...
# SUSFS v2.3 uses void* for the execve argument containers at the API boundary.
# Some SukiSU trees carry older struct user_arg_ptr* declarations/definitions,
# which Clang diagnoses as conflicting types even though the printed signatures
# look nearly identical. Normalize both sides to the upstream ABI.
python3 - "$sucompat_h" <<'PYEXECAPI'
from pathlib import Path
import re, sys
...
s = re.sub(
    r'(^\s*(?:extern\s+)?int\s+ksu_handle_execveat\s*\(\s*int\s*\*fd\s*,\s*struct\s+filename\s*\*\*filename_ptr\s*,\s*)(?:struct\s+user_arg_ptr\s*\*|void\s*\*)argv\s*,\s*(?:struct\s+user_arg_ptr\s*\*|void\s*\*)envp\s*,\s*int\s*\*flags\s*\)\s*;$',
    r'\1void *argv, void *envp, int *flags);', s, flags=re.M)

s = re.sub(
    r'(^\s*(?:extern\s+)?int\s+ksu_handle_execveat_sucompat\s*\(\s*int\s*\*fd\s*,\s*struct\s+filename\s*\*\*filename_ptr\s*,\s*)(?:struct\s+user_arg_ptr\s*\*|void\s*\*)argv(?:_user)?\s*,\s*(?:struct\s+user_arg_ptr\s*\*|void\s*\*)envp(?:_user)?\s*,\s*int\s*\*[^)]*\)\s*;$',
    r'\1void *argv_user, void *envp_user, int *__never_use_flags);', s, flags=re.M)
...
HEOF
  ...
  if [ -f "$ksud_integration_c" ]; then
    # SUSFS v2.3 replaces the legacy 2-argument ksud execve helper call with
    # the 5-argument execveat hook. The old call site is obsolete; remove it
    # using Python rather than sed so the literal '&' cannot be mis-parsed.
    python3 - "$ksud_integration_c" <<'PYEXECFIX'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()
old_call = r'ksu_handle_execveat_ksud\(\s*path\s*,\s*&argv\s*\)\s*;'
s, n = re.subn(old_call, '', s)
if n:
    p.write_text(s)
PYEXECFIX

    # SUSFS v2.3 moved fstat handling to ksu_handle_vfs_fstat(). Some SukiSU
    # trees retain the body tail but lose these local declarations during the
    # legacy syscall-hook cleanup. Restore them at the function entry.
    python3 - "$ksud_integration_c" <<'PYFSTAT'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()

# Match the function by name and inspect its parameter list. Do not try to
# locate the end of the function with a naive first-'}' search: the body
# contains nested blocks, which caused the previous repair to miss the locals.
pat = r'(\b(?:static\s+)?(?:long|int|void)\s+ksu_handle_vfs_fstat\s*\(([^{};]*)\)\s*\{)'
m = re.search(pat, s, flags=re.S)
if m:
    signature = m.group(2)
    body_start = m.end()
    if re.search(r'\bkstat_size_ptr\b', signature):
        # Insert immediately after the opening brace, before any nested block.
        # This is deterministic and keeps both variables in function scope.
        prefix = s[:body_start]
        rest = s[body_start:]
        if not re.match(r'\s*loff_t\s+orig_size\s*=\s*\*kstat_size_ptr\s*;', rest):
            s = prefix + '\n    loff_t orig_size = *kstat_size_ptr;\n    size_t extra = 0;' + rest
            p.write_text(s)
    else:
        print(f'::warning::ksu_handle_vfs_fstat() found without kstat_size_ptr parameter in {p}; skipping local repair')
else:
    # A tree may have already removed the v2.2 fstat helper entirely. That is
    # valid for SUSFS v2.3; do not abort the whole patch pipeline.
    print(f'::warning::ksu_handle_vfs_fstat() is absent in {p}; skipping legacy fstat-local repair')
PYFSTAT

    # Fail here instead of allowing the compiler to rediscover these known
    # legacy leftovers.
    if grep -qE '^[[:space:]]*ksu_handle_execveat_ksud[[:space:]]*\([[:space:]]*path[[:space:]]*,[[:space:]]*&argv[[:space:]]*\)[[:space:]]*;' "$ksud_integration_c"; then
      echo "::error::Legacy 2-argument ksu_handle_execveat_ksud call remains in $ksud_integration_c"
      grep -n 'ksu_handle_execveat_ksud' "$ksud_integration_c" || true
      exit 1
    fi

    # Do not require ksu_handle_vfs_fstat() to exist. SUSFS v2.3 trees may
    # already have removed the legacy helper entirely. If it exists, the
    # Python repair above handles the locals; otherwise continue normally.
    if grep -qE 'ksu_handle_vfs_fstat[[:space:]]*\(' "$ksud_integration_c"; then
      echo "SUSFS fstat helper present; compatibility validation complete"
    else
      echo "SUSFS v2.3 fstat helper absent; no legacy fstat repair required"
    fi

    if grep -qE 'ksu_handle_execveat_ksud[[:space:]]*\([[:space:]]*path[[:space:]]*,[[:space:]]*&argv[[:space:]]*\)' "$ksud_integration_c"; then
      echo "::error::Legacy 2-argument ksu_handle_execveat_ksud call remains in $ksud_integration_c"
      grep -n 'ksu_handle_execveat_ksud' "$ksud_integration_c" || true
      exit 1
    fi
...
# -----------------------------------------------------------------------------
# SukiSU adb_root.c reject compatibility
#
# Some SUSFS enable patches carry an older adb_root.c hunk which replaces the
# legacy ksud execve hooks with the newer VFS-fstat handling. On SukiSU trees
# that already contain the newer implementation, that hunk rejects even though
# there is nothing left to apply. Do not blindly delete the reject: verify the
# source state first.
# -----------------------------------------------------------------------------
if [ -f kernel/feature/adb_root.c.rej ]; then
  target="kernel/feature/adb_root.c"
  rej="kernel/feature/adb_root.c.rej"

  if [ ! -f "$target" ]; then
    echo "::error::adb_root.c.rej exists but kernel/feature/adb_root.c is missing"
    exit 1
  fi

  # SUSFS 60024a4 contains the adb_root execve -> execveat ABI conversion.
  # SukiSU may partially apply this hunk before the final function-boundary
  # hunk rejects, so do not require the target to match one exact historical
  # SukiSU signature. Reconcile all known intermediate states below.
  if grep -Fq 'setup_ld_preload(envp_user_ptr)' "$rej" && \
     grep -Fq 'do_ksu_adb_root_handle_execveat(filename, envp_user_ptr)' "$rej"; then

    echo "  adb_root.c: reconciling SUSFS execve -> execveat ABI reject"

    # 1. Convert the internal handler declaration, regardless of the exact
    #    legacy parameter spelling used by the SukiSU base tree.
    if grep -qE '^[[:space:]]*static[[:space:]]+long[[:space:]]+do_ksu_adb_root_handle_execve[[:space:]]*\(' "$target"; then
      perl -0pi -e 's/static long do_ksu_adb_root_handle_execve\s*\([^)]*\)/static long do_ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)/g' "$target"
    fi

    # 2. Normalize references inside the converted handler.
    perl -0pi -e 's/\bis_exec_adbd\(\s*filename_user\s*\)/is_exec_adbd(filename)/g; s/setup_ld_preload\(\s*regs\s*,\s*envp_p\s*\)/setup_ld_preload(envp_user_ptr)/g' "$target"

    # 3. If the internal handler is already converted, normalize its name.
    perl -0pi -e 's/\bdo_ksu_adb_root_handle_execve\s*\(/do_ksu_adb_root_handle_execveat(/g' "$target"

    # 4. Replace the old pt_regs wrapper pair in-place. Keeping the replacement
    #    at the original location avoids appending C code after a trailing
    #    #endif/#ifdef in some SukiSU trees.
    perl -0pi -e '
      s/\nlong\s+ksu_adb_root_handle_execve\s*\(\s*struct\s+pt_regs\s*\*regs\s*\)\s*\{.*?\n\}\s*\n*long\s+ksu_adb_root_handle_execveat\s*\(\s*struct\s+pt_regs\s*\*regs\s*\)\s*\{.*?\n\}/
...
long ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)
{
    ...
    if ! grep -qE 'long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const[[:space:]]+char[[:space:]]+\*filename,[[:space:]]*void[[:space:]]+\*\*\*envp_user_ptr[[:space:]]*\)' "$target"; then
      # Extremely old/partially-repaired trees may have no wrapper left.
      # Insert before the final preprocessor terminator when one exists;
      # otherwise append as a normal file-scope function.
      wrapper='
long ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)
{
    if (static_branch_unlikely(&ksu_adb_root)) {
...
'
      if tail -n 1 "$target" | grep -q '^#endif'; then
        perl -0pi -e 's/\n(#endif[^\n]*\n?)\z/\nWRAPPER_PLACEHOLDER\n$1/s' "$target"
        perl -0pi -e 's/WRAPPER_PLACEHOLDER/'"$wrapper"'/s' "$target"
      else
        printf '%s\n' "$wrapper" >> "$target"
      fi
    fi

    # 5. SUSFS v2.3.0 applies the selected root profile after adb-root setup.
    if grep -q 'escape_to_root_for_adb_root();' "$target" && \
       ! grep -q 'escape_with_root_profile() failed' "$target"; then
      perl -0pi -e 's/(escape_to_root_for_adb_root\(\);\n)([ \t]*return 0;\n[ \t]*\})/$1\n    ret = escape_with_root_profile();\n    if (ret)\n        pr_err("escape_with_root_profile() failed: %d\\n", (int)ret);\n\n$2/s' "$target"
    fi

    # 6. Verify the actual target, not merely the reject contents.
    if grep -qE 'long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const[[:space:]]+char[[:space:]]+\*filename,[[:space:]]*void[[:space:]]+\*\*\*envp_user_ptr[[:space:]]*\)' "$target" && \
       grep -q 'do_ksu_adb_root_handle_execveat(filename, envp_user_ptr)' "$target"; then
      echo "  adb_root.c: SUSFS execveat ABI reconciled successfully"
      rm -f "$rej"
    else
      echo "::error::SUSFS adb_root reject was recognized but ABI reconciliation did not produce the required target"
      cat "$rej"
      exit 1
    fi

  elif grep -qE 'ksu_handle_vfs_fstat' "$rej" && \
       grep -qE 'ksu_handle_vfs_fstat' "$target"; then
    echo "  adb_root.c: SUSFS vfs-fstat implementation already present; reject is obsolete"
    rm -f "$rej"
  else
    echo "::error::Unknown adb_root.c reject content; refusing to discard it"
    cat "$rej"
    exit 1
  fi
fi
...
# =============================================================================
# SUSFS v2.3.0 final API reconciliation
# =============================================================================
# The SUSFS v2.3 patch converts sucompat.c from the old pt_regs/3-argument
# execveat bridge to the VFS-based 5-argument execveat API. Earlier compatibility
# helpers in this workflow could leave both implementations in the same file,
# producing duplicate/conflicting definitions. Reconcile the generated tree once,
# after every compatibility helper has run, immediately before the safety sweep.
finalize_susfs_v23_apis() {
  [ "${SUSVER:-}" = "v2.3.0" ] || return 0

  for base in "kernel" "drivers/kernelsu"; do
    local c="$base/feature/sucompat.c"
    local h="$base/feature/sucompat.h"
    [ -f "$c" ] || continue
...
def remove_function(src, pattern):
    while True:
...
# SUSFS v2.3 owns the 5-argument VFS execveat handler. Remove the old
# pt_regs/3-argument handler if a previous compatibility pass left it behind.
s = remove_function(
    s,
    r'^\s*long\s+ksu_handle_execveat_sucompat\s*\(\s*'
    r'const\s+char\s+__user\s*\*\*filename_user\s*,\s*'
    r'int\s+orig_nr\s*,\s*struct\s+pt_regs\s*\*regs\s*\)\s*'
)

# The SUSFS patch uses a single static su_path. Keep the first definition and
# delete duplicates introduced by compatibility helpers.
seen = False
out = []
for line in s.splitlines():
    if re.match(r'^\s*static\s+const\s+char\s+su_path\s*\[\]\s*=\s*SU_PATH\s*;\s*$', line):
        if seen:
            continue
        seen = True
    out.append(line)
...
if h.exists():
    hs = h.read_text()
    # Remove old/conflicting multiline declarations for execveat_sucompat.
    hs = re.sub(
        r'(?ms)^\s*(?:long|int)\s+ksu_handle_execveat_sucompat\s*\(.*?\)\s*;\s*',
        '', hs)
    hs = re.sub(
        r'(?ms)^\s*(?:long|int)\s+ksu_handle_execveat\s*\(.*?\)\s*;\s*',
        '', hs)
    ...
    if idx is None:
        raise SystemExit(f'sucompat.h has no final #endif: {h}')
    lines[idx:idx] = [execveat_sucompat, execveat]
    h.write_text('\n'.join(lines).rstrip() + '\n')

    # Never leave an old 3-argument declaration behind in malformed/continued lines.
    final = h.read_text()
    if re.search(r'long\s+ksu_handle_execveat_sucompat\s*\(\s*const\s+char', final):
        raise SystemExit(f'old execveat_sucompat declaration remains: {h}')

PYV23

    # SUSFS v2.3 removes the legacy ksu_close_fd helper together with the
    # old pt_regs execve handler.  Some SukiSU revisions leave only the
    # cleanup call behind after the handler body is reconciled.  Remove that
    # orphaned call rather than replacing it with an unrelated fd API.
    if grep -qE '\bksu_close_fd\s*\(' "$c"; then
      sed -i '/^[[:space:]]*ksu_close_fd[[:space:]]*(tmp_fd);[[:space:]]*$/d' "$c"
    fi

    if grep -qE '\bksu_close_fd\s*\(' "$c"; then
      echo "::error::Legacy ksu_close_fd reference remains after SUSFS v2.3 reconciliation: $c"
      grep -n 'ksu_close_fd' "$c" || true
      exit 1
    fi

    echo "✅ SUSFS v2.3.0 sucompat API reconciled: $c"
  done

  # SUSFS v2.3 removes the old syscall_event_bridge path from KernelSU Kbuild.
  # Leaving it compiled pulls obsolete pt_regs APIs back into the build.
  for kbuild in \
    "$KSU_FOLDER/kernel/Kbuild" \
    "$KSU_FOLDER/kernel/Makefile" \
    ...
    if [ -f "$kbuild" ]; then
      sed -i '/hook\/syscall_event_bridge\.o/d' "$kbuild"
      sed -i '/feature\/syscall_event_bridge\.o/d' "$kbuild"
    fi
  done

  echo "✅ SUSFS v2.3.0 obsolete syscall_event_bridge object disabled"
}
...
# Final ksud_integration.c repair.
# SUSFS 10_enable_susfs_for_ksu.patch moves the legacy execve/fstat syscall-hook
# block to the VFS-facing ksu_handle_vfs_fstat() helper. On SukiSU trees with
# source skew, that large hunk can reject while the rest of the patch applies.
# Recover the VFS-fstat shape from the current legacy block instead of merely
# declaring variables that belong to a function which does not exist.
if [ -f drivers/kernelsu/runtime/ksud_integration.c ]; then
  python3 - drivers/kernelsu/runtime/ksud_integration.c <<'PYFINALKSUD'
from pathlib import Path
import re, sys
...
m_exec = re.search(r'\b(?:static\s+)?(?:void|long|int)\s+ksu_execve_hook_ksud_common\s*\([^;{]*\)\s*\{', s, re.S)
if m_exec:
    a,b,body = body_after(s, m_exec)
    decl = re.compile(r'^\s*struct\s+user_arg_ptr\s+argv\s*=\s*\{\s*\.ptr\.native\s*=\s*argv_user\s*\};\s*\n?', re.M)
    if decl.search(body) and len(re.findall(r'\bargv\b', body)) == 1:
        body = decl.sub('', body, count=1)
        s = s[:a] + body + s[b:]

m = re.search(r'\bvoid\s+ksu_handle_vfs_fstat\s*\(\s*int\s+fd\s*,\s*loff_t\s*\*\s*kstat_size_ptr\s*\)\s*\{', s)
if m:
    a,b,body = body_after(s, m)
    prefix=[]
    if not re.search(r'\bloff_t\s+orig_size\s*=\s*\*kstat_size_ptr\s*;', body):
        prefix.append('    loff_t orig_size = *kstat_size_ptr;')
    if not re.search(r'\bsize_t\s+extra\s*=\s*0\s*;', body):
        prefix.append('    size_t extra = 0;')
    if prefix:
        s=s[:a]+'\n'+'\n'.join(prefix)+s[a:]
    p.write_text(s)
    print('ksud_integration.c: existing VFS fstat helper repaired')
    raise SystemExit(0)

# Some SukiSU revisions have already deleted both the old syscall wrapper and
# the execve common helper, while a rejected SUSFS hunk leaves only the fstat
# adjustment statements in the file.  In that state the previous reconstruction
# cannot find ksu_sys_fstat(). Detect the orphaned VFS-fstat statements directly.
if re.search(r'\bextra\s*=\s*ksu_rc_len\s*\+\s*module_rc_len\s*;', s) and \
   re.search(r'\*kstat_size_ptr\s*=\s*orig_size\s*\+\s*extra\s*;', s):
    func = """void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)
{
    loff_t orig_size = *kstat_size_ptr;
    size_t extra = 0;
    bool is_rc = false;
...
"""
    # Remove the orphaned statements and their nearby partial block, then add
    # the complete helper immediately before the module-init section.
    orphan = re.compile(
        r'(?s)\n?\s*(?:if\s*\(\s*is_rc\s*\)\s*\{)?\s*'
        r'extra\s*=\s*ksu_rc_len\s*\+\s*module_rc_len\s*;\s*'
        r'\*kstat_size_ptr\s*=\s*orig_size\s*\+\s*extra\s*;\s*'
        r'pr_info\s*\(\s*\"adding rc len:.*?\)\s*;\s*'
        r'\}?'
    )
    s, n = orphan.sub('\n', s, count=1)
    if not n:
        raise SystemExit('ERROR: orphaned VFS-fstat statements were detected but could not be isolated')
    anchor = re.search(r'(?m)^\s*//\s*ksud:\s*module support\s*$', s)
    if not anchor:
        raise SystemExit('ERROR: cannot locate ksud module-support anchor for VFS-fstat helper')
    s = s[:anchor.start()] + func + s[anchor.start():]
    p.write_text(s)
    print('ksud_integration.c: reconstructed SUSFS VFS fstat helper from orphaned fstat statements')
    raise SystemExit(0)

start_pat = re.compile(r'\b(?:static\s+)?(?:void|long|int)\s+ksu_execve_hook_ksud_common\s*\([^;{]*\)\s*\{', re.S)
end_pat = re.compile(r'\nstatic\s+int\s+input_handle_event_handler_pre\s*\(', re.S)
ms = start_pat.search(s)
me = end_pat.search(s, ms.end() if ms else 0)
if ms and me and re.search(r'\b(?:static\s+)?long\s+ksu_sys_fstat\s*\(', s[ms.start():me.start()]):
    replacement = """void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)
{
    loff_t orig_size = *kstat_size_ptr;
    size_t extra = 0;
    bool is_rc = false;
...
"""
    s = s[:ms.start()] + replacement + s[me.start()+1:]
    p.write_text(s)
    print('ksud_integration.c: reconstructed SUSFS VFS fstat helper from legacy hook region')
    raise SystemExit(0)

# The common drivers/kernelsu mirror can be ahead of/behind the KernelSU tree.
# If both historical fstat anchors are gone, install the VFS-facing helper at
# the module-support boundary instead of aborting.
anchor = re.search(r'(?m)^\s*//\s*ksud:\s*module support\s*$', s)
if anchor:
    func = '''void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)
{
...
'''
    s = s[:anchor.start()] + func + s[anchor.start():]
    p.write_text(s)
    print('ksud_integration.c: installed missing SUSFS VFS fstat helper at module-support boundary')
    raise SystemExit(0)

raise SystemExit('ERROR: cannot locate ksud module-support anchor for SUSFS VFS-fstat helper recovery')
PYFINALKSUD

  if ! grep -qE 'void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\([^)]*loff_t[[:space:]]*\*[[:space:]]*kstat_size_ptr' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::SUSFS v2.3 ksu_handle_vfs_fstat() is missing after repair"
    exit 1
  fi
  if ! grep -qE 'loff_t[[:space:]]+orig_size[[:space:]]*=[[:space:]]*\*kstat_size_ptr[[:space:]]*;' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::ksu_handle_vfs_fstat(): orig_size declaration missing"
    exit 1
  fi
  if ! grep -qE 'size_t[[:space:]]+extra[[:space:]]*=\s*0[[:space:]]*;' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::ksu_handle_vfs_fstat(): extra declaration missing"
    exit 1
  fi
  # Keep the mirror runtime header synchronized with the VFS helper body.
  if [ -f drivers/kernelsu/runtime/ksud.h ] && ! grep -qE '^void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\(' drivers/kernelsu/runtime/ksud.h; then
    printf '\nvoid ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr);\n' >> drivers/kernelsu/runtime/ksud.h
    echo "✅ Added ksu_handle_vfs_fstat() declaration to drivers/kernelsu/runtime/ksud.h"
  fi
  echo "✅ Final ksud_integration.c SUSFS v2.3 VFS-fstat conversion validated"
fi
...
for sucompat_h in \
  ...
  "$COMMON_KERNEL_FOLDER/drivers/kernelsu/feature/sucompat.h"; do
  if [ -f "$sucompat_h" ]; then
    # Repair legacy runs that appended compatibility declarations after the
    # guard (for example '#endiflong ...') before enforcing the final check.
    python3 - "$sucompat_h" <<'PY2'
from pathlib import Path
import re, sys
...
# Split malformed '#endif<declaration>' lines into a real guard terminator
# followed by the declaration, then move known compatibility declarations
# back inside the guard.
decls = []
for line in lines:
    m = re.match(r'^#endif(?:\s*)(long\s+ksu_handle_(?:faccessat|stat)_sucompat\s*\([^;]+;)$', line.strip())
    if m:
...
for line in lines:
    stripped = line.strip()
    if stripped.startswith('long ksu_handle_faccessat_sucompat(') or stripped.startswith('long ksu_handle_stat_sucompat('):
...
seen = set()
decls = [d for d in decls if not (d in seen or seen.add(d))]

# Locate the final standalone #endif and insert declarations immediately before it.
idx = None
for i in range(len(clean) - 1, -1, -1):
    if clean[i].strip() == '#endif':
        idx = i
        break
...
for sucompat_c in \
  ...
  if [ -f "$sucompat_c" ]; then
# Normalize any legacy struct user_arg_ptr* execveat definitions to the SUSFS
# v2.3 ABI used by fs/exec.c and the matching sucompat.h declarations.
python3 - "$sucompat_c" <<'PYEXECDEF'
from pathlib import Path
import re, sys
...
s = re.sub(
    r'(^[ \t]*int\s+ksu_handle_execveat\s*\(\s*int\s*\*fd\s*,\s*struct\s+filename\s*\*\*filename_ptr\s*,\s*)(?:struct\s+user_arg_ptr\s*\*|void\s*\*)argv\s*,\s*(?:struct\s+user_arg_ptr\s*\*|void\s*\*)envp\s*,\s*int\s*\*flags\s*\)\s*\{',
    r'\1void *argv, void *envp, int *flags)\n{', s, flags=re.M)

s = re.sub(
    r'(^[ \t]*int\s+ksu_handle_execveat_sucompat\s*\(\s*int\s*\*fd\s*,\s*struct\s+filename\s*\*\*filename_ptr\s*,\s*)(?:struct\s+user_arg_ptr\s*\*|void\s*\*)argv(?:_user)?\s*,\s*(?:struct\s+user_arg_ptr\s*\*|void\s*\*)envp(?:_user)?\s*,\s*int\s*\*[^)]*\)\s*\{',
    r'\1void *argv_user, void *envp_user, int *__never_use_flags)\n{', s, flags=re.M)
