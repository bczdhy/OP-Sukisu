...
fix_op61_selinux_backup_lifecycle() {
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then return 0; fi
  local target="$1"; [ -f "$target" ] || return 0

  echo "Applying OP 6.1 SELinux-hide backup lifecycle fix in: $target"

  # SukiSU normally frees backup_sepolicy at boot_completed when the feature
  # was not running. On this OP13R Android 14/6.1 build the Manager can issue
  # SET_FEATURE(SELINUX_HIDE=1) after boot, so freeing the snapshot makes the
  # feature permanently return -EAGAIN (-11). Keep the clean snapshot alive
  # for the runtime toggle. It is still freed by selinux_hide_exit() on a
  # kernel/module shutdown path.
  python3 - "$target" <<'PY_OP61_BACKUP'
from pathlib import Path
import sys
p = Path(sys.argv[1])
s = p.read_text()
old = 'ksu_selinux_hide_drop_backup_if_unused();'
if old not in s:
    if 'OP 6.1: keeping backup_sepolicy alive for runtime SELinux-hide toggle' in s:
        print('OP 6.1: backup lifecycle fix already present in', p)
        raise SystemExit(0)
    print('OP61: boot_event backup drop call not found in', p)
    raise SystemExit(0)
s = s.replace(old, '/* OP 6.1: keep backup_sepolicy for post-boot SELinux-hide toggle. */', 1)
p.write_text(s)
print('OP 6.1: keeping backup_sepolicy alive for runtime SELinux-hide toggle in', p)
PY_OP61_BACKUP
}
...
fix_op61_selinux_backup_lifecycle "kernel/runtime/boot_event.c"
...
fix_op61_selinux_backup_lifecycle "drivers/kernelsu/runtime/boot_event.c"
