...
# Normalize the v2.3 VFS execveat implementation signatures to the exact
# SUSFS ABI.  Older SukiSU/SUSFS passes may leave struct user_arg_ptr * in
# the definition while the v2.3 header exposes void *; clang treats those
# as conflicting function types even though the call sites are compatible.
s = re.sub(
    r'(int\s+ksu_handle_execveat_sucompat\s*\(\s*'
    r'int \*fd,\s*struct filename \*\*filename_ptr,\s*)'
    r'struct user_arg_ptr \*argv(?:_user)?\s*,\s*'
    r'struct user_arg_ptr \*envp(?:_user)?\s*,\s*'
    r'int \*flags\s*\)',
    r'\1void *argv_user, void *envp_user, int *flags)',
    s,
    flags=re.S,
)
s = re.sub(
    r'(int\s+ksu_handle_execveat\s*\(\s*'
    r'int \*fd,\s*struct filename \*\*filename_ptr,\s*)'
    r'struct user_arg_ptr \*argv(?:_user)?\s*,\s*'
    r'struct user_arg_ptr \*envp(?:_user)?\s*,\s*'
    r'int \*flags\s*\)',
...
exit 1
  ...
  if [ -f drivers/kernelsu/runtime/ksud_integration.c ] && [ "${SUSVER:-}" = "v2.3.0" ]; then
    # SUSFS v2.3 replaces the old 2-argument ksud execve bridge with the
    # VFS-based 5-argument ksu_handle_execveat_ksud() API.  A partially
    # applied SukiSU hunk can leave the old wrapper call behind:
    #     ksu_handle_execveat_ksud(path, &argv);
    # That call cannot be adapted safely because the old wrapper does not have
    # the struct filename/envp/flags objects required by the new ABI.  The
    # obsolete wrapper call is therefore removed; the new VFS hook is invoked
    # through ksu_handle_execveat() by the SUSFS v2.3 path.
    python3 - drivers/kernelsu/runtime/ksud_integration.c <<'PYV23KSUD'
from pathlib import Path
import re
import sys
...
# Remove the exact legacy wrapper call left by older SukiSU trees.
s = re.sub(
    r'^[ \t]*ksu_handle_execveat_ksud\(path, \&argv\);[ \t]*\n',
    '',
    s,
    flags=re.M,
)

# SUSFS v2.3's fstat conversion can leave the new accounting tail detached
# from its declarations when an older SukiSU hunk was already present.  The
# detached statements are not valid in that old handler and refer to symbols
# that do not exist there.  Remove only that exact orphaned accounting tail;
# a correctly formed ksu_handle_vfs_fstat() is left untouched.
s = re.sub(
    r'^[ \t]*extra = ksu_rc_len \+ module_rc_len;[ \t]*\n'
    r'^[ \t]*\*kstat_size_ptr = orig_size \+ extra;[ \t]*\n'
    r'^[ \t]*pr_info\("adding rc len: %lld -> %lld \(static=%zu module=%zu\)", orig_size, \*kstat_size_ptr, ksu_rc_len, module_rc_len\);[ \t]*\n',
    '',
    s,
    flags=re.M,
)
...
PYV23KSUD
  ...
  if [ -f drivers/kernelsu/runtime/ksud_integration.c ] && [ "${SUSVER:-}" = "v2.3.0" ]; then
    # Final SUSFS v2.3 ksud integration cleanup. The upstream patch removes
    # the old pt_regs fstat wrapper and keeps a void VFS helper. Rebuild that
    # helper if a partially-applied SukiSU hunk left stale locals or a broken
    # body.
    python3 - drivers/kernelsu/runtime/ksud_integration.c <<'PYV23FSTAT'
from pathlib import Path
import re
import sys
...
s = re.sub(
    r'^[ \t]*struct user_arg_ptr argv = \{ \.ptr\.native = argv_user \};[ \t]*\n',
    '', s, flags=re.M)
...
helper = """void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)
{
...
# Remove obsolete pre-VFS exec wrappers completely; their argv/stat locals are
# no longer part of SUSFS v2.3's runtime path.
for sig in (
    'static void ksu_execve_hook_ksud_common(',
    'void ksu_execve_hook_ksud(',
    'void ksu_execveat_hook_ksud(',
    'static long ksu_sys_fstat(',
):
    s = replace_function(s, sig, '')

# Replace an already-created VFS helper, or insert one if the old fstat helper
# was removed by the previous loop.
if 'void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)' in s:
    s = replace_function(s, 'void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr)', helper)
else:
    anchor = s.find('// ksud: module support')
    if anchor >= 0:
        s = s[:anchor] + helper + '\n' + s[anchor:]
