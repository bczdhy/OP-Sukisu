...
ensure_backup_sepolicy_symbol() {
  ...
  # selinux_hide.c is compiled as its own translation unit.  Make the symbol
  # visible there even when this SukiSU revision does not pull ksu.h's extern
  # through the expected include path.  Do not define the object here.
  if ! grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*backup_sepolicy[[:space:]]*;' "$cfile"; then
    ...
    echo "  ✅ Added direct backup_sepolicy extern to $(basename "$cfile")"
  fi
}
...
# =============================================================================
# SukiSU-Ultra 40939 / SUSFS v2.3 compatibility cleanup
# =============================================================================
# The 60024a4 SUSFS patch converts sucompat from the legacy syscall-hook API
# to the execveat/VFS API. SukiSU-Ultra 40939 still carries portions of the
...
fix_sukisu_ultra_40939_api() {
  ...
  echo "Reconciling SukiSU-Ultra 40939 SUSFS execve API in: $base"
...
PY_REMOVE_LEGACY_EXECVE
  ...
  # Make the bridge stop calling symbols removed by SUSFS. The actual execve
  # transformation now occurs through fs/exec.c -> ksu_handle_execveat(); the
  # syscall bridge must only dispatch the original syscall and retain its init
  # bookkeeping. This also avoids passing userspace filename pointers into the
  # new kernel-space adb_root API.
  if [ -f "$bridge" ]; then
    python3 - "$bridge" <<'PY_BRIDGE_40939'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()
...
def replace_function(src, name, replacement):
    m = re.search(r'^long __nocfi ' + re.escape(name) + r'\s*\([^)]*\)\s*\{', src, re.M)
    if not m:
        return src, False
    start = m.start(); brace = src.find('{', m.start())
    depth = 0; i = brace; in_str=None; esc=False; in_line=False; in_block=False
    while i < len(src):
...
replacement = """long __nocfi ksu_hook_execve(int orig_nr, const struct pt_regs *regs)
{
    const char __user **filename_user = (const char __user **)&PT_REGS_PARM1(regs);
    long ret;

    /*
     * SUSFS v2.3 moved execve handling to the filename-based execveat path.
     * Keep only the init/tracepoint bookkeeping here; fs/exec.c invokes
     * ksu_handle_execveat() with struct filename/user_arg_ptr objects.
     */
...
"""
s, _ = replace_function(s, 'ksu_hook_execve', replacement)

# If the old bridge was left without a compatible setresuid migration, use the
# current syscall arguments directly (SUSFS defines the 3-argument handler).
s = re.sub(
    r'ksu_handle_setresuid\s*\(\s*old_uid\s*,\s*current_uid\(\)\.val\s*\)',
    'ksu_handle_setresuid((uid_t)PT_REGS_PARM1(regs), (uid_t)PT_REGS_PARM2(regs), (uid_t)PT_REGS_PARM3(regs))',
    s)
s = re.sub(r'^\s*uid_t old_uid __maybe_unused = current_uid\(\)\.val;\s*\n', '', s, flags=re.M)

# Some SukiSU-Ultra 40939 trees contain an additional ksud execveat helper
# that is not present in newer bridge revisions. Remove those obsolete helper
# definitions/calls before validating the bridge. The SUSFS v2.3 exec path is
# handled from fs/exec.c, so these syscall-bridge helpers must not remain.
def remove_named_functions(src, names):
    for name in names:
        pat = re.compile(r'(?m)^[ \t]*(?:static[ \t]+)?(?:long|int|void)[ \t]+__nocfi[ \t]+\b' + re.escape(name) + r'\b[^\n]*\{')
        # Also match definitions without __nocfi.
        pat2 = re.compile(r'(?m)^[ \t]*(?:static[ \t]+)?(?:long|int|void)[ \t]+\b' + re.escape(name) + r'\b[^\n]*\{')
        for rx in (pat, pat2):
            while True:
                m = rx.search(src)
                if not m:
...
s = remove_named_functions(s, ('ksu_execveat_hook_ksud', 'ksu_execve_hook_ksud'))

# Remove direct calls to obsolete ksud exec hooks. These helpers are not part of
# the SUSFS v2.3 execveat bridge and retaining a call recreates an implicit
# declaration/link dependency.
s = re.sub(r'(?m)^[ \t]*(?:if[^\n]*\n[ \t]*)?ksu_execve(?:at)?_hook_ksud\s*\([^;]*\);[ \t]*\n', '', s)

# SUSFS v2.3 removed the old root-exec sulog capture API:
#   ksu_sulog_capture_root_execve(...)
# The pending event was only used to emit that obsolete root-exec event, so
# remove the declaration, capture assignment, and corresponding emit calls as
# one unit. This prevents the next legacy-symbol check from stopping the build
# on SukiSU-Ultra 40939's older bridge implementation.
s = re.sub(
    r'(?m)^[ \t]*struct ksu_sulog_pending_event \*pending_root_execve = NULL;[ \t]*\n',
    '', s)
s = re.sub(
    r'(?m)^[ \t]*pending_root_execve\s*=\s*ksu_sulog_capture_root_execve\([^;]*\);[ \t]*\n',
    '', s)
s = re.sub(
    r'(?m)^[ \t]*ksu_sulog_emit_pending\(pending_root_execve,[^;]*\);[ \t]*\n',
    '', s)

# SukiSU-Ultra 40939 can also leave the old adb-root syscall bridge in
# syscall_event_bridge.c.  SUSFS 60024a4 moved adb-root handling into the
# filename-based execveat path in sucompat/adb_root, so the legacy bridge must
# not call the old pt_regs API.  Preserve the surrounding return variable but
# remove only the obsolete conditional/direct calls.
s = re.sub(
    r'(?m)^[ \t]*ret[ \t]*=[ \t]*execveat[ \t]*\?[ \t]*ksu_adb_root_handle_execveat\s*\([^;\n]*\)[ \t]*:[ \t]*ksu_adb_root_handle_execve\s*\([^;\n]*\)[ \t]*;[ \t]*$',
    'ret = 0;', s)
s = re.sub(
    r'(?m)^[ \t]*(?:ksu_adb_root_handle_execveat|ksu_adb_root_handle_execve)\s*\([^;\n]*\)[ \t]*;[ \t]*$',
    '', s)

# SukiSU-Ultra 40939 can retain a direct call to the removed legacy
# ksu_handle_execve_sucompat() bridge even after its old function definition has
# been removed.  The SUSFS v2.3 exec path is now entered through
# ksu_handle_execveat(), so neutralize only this obsolete invocation.
# Match balanced parentheses so multiline calls are handled as well.
def replace_call(src, name, replacement='0'):
    ...
    while True:
        ...
        if not m:
            return src
        i = m.end() - 1
        depth = 0; j = i; in_str = None; esc = False; in_line = False; in_block = False
        while j < len(src):
            ch = src[j]; nxt = src[j+1] if j + 1 < len(src) else ''
            if in_line:
                if ch == '\n': in_line = False
            elif in_block:
                if ch == '*' and nxt == '/': in_block = False; j += 1
            elif in_str:
                if esc: esc = False
                elif ch == '\\': esc = True
                elif ch == in_str: in_str = None
            else:
                if ch == '/' and nxt == '/': in_line = True; j += 1
                elif ch == '/' and nxt == '*': in_block = True; j += 1
                elif ch in ('\"', "'"): in_str = ch
                elif ch == '(' : depth += 1
                elif ch == ')':
                    depth -= 1
                    if depth == 0:
                        src = src[:m.start()] + replacement + src[j+1:]
                        break
            j += 1
        else:
            return src

s = replace_call(s, 'ksu_handle_execve_sucompat')

# Do not leave references to SUSFS-removed exec/sulog bridge symbols.
for sym in ('ksu_execveat_hook_ksud', 'ksu_execve_hook_ksud', 'ksu_sulog_capture_root_execve', 'ksu_adb_root_handle_execve(', 'ksu_handle_execve_sucompat('):
    if re.search(r'\b' + re.escape(sym), s):
        raise SystemExit(f'legacy bridge symbol remains: {sym}')
p.write_text(s)
PY_BRIDGE_40939
  fi
...
PY_HEADER_40939
  ...
  # SUSFS v2.3 removes the legacy syscall-event bridge objects from Kbuild.
  # Keeping syscall_event_bridge.o compiled defeats the SUSFS migration and
  # reintroduces the old execve/pt_regs API. Remove only that obsolete object.
  for kb in "$base/Kbuild" "$base/Makefile"; do
    if [ -f "$kb" ]; then
      sed -i '/^[[:space:]]*kernelsu-objs[[:space:]]*+=[[:space:]]*hook\/syscall_event_bridge\.o[[:space:]]*$/d' "$kb"
    fi
  done
...
PY_EXECVEAT_HDR
  ...
  echo "✅ SukiSU-Ultra 40939 SUSFS execve API normalized in: $base"
}
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# Reconcile known SUSFS KernelSU reject hunks against SukiSU 4.2.x.
#
# The 60024a4 SUSFS patch is generated against a specific KernelSU source
# revision.  SukiSU 40939 differs in these three remaining regions.  Do not
...
apply_known_susfs_reject() {
  ...
  # SukiSU 40939 has a different adb-root wrapper layout from the KernelSU
