...
# kernel/Kbuild.rej is expected for the OP13R/SukiSU 6.1 combination.
# The SUSFS enable patch contains a Kbuild hunk that removes hook/syscall objects
# which are intentionally retained by SukiSU and restored/used by this build.
# Do not edit Kbuild to satisfy that rejected hunk; discard the reject only after
# the SukiSU compatibility pass has had a chance to reconcile the real source.
...
# Finalize the staging KernelSU tree before changing cwd.
# The reject files below are known consequences of applying the SUSFS patch to
# this SukiSU revision: their corresponding source is deliberately kept or
# restored by the compatibility functions above. Any other staging reject is
# still fatal. Use an absolute tree path so this check cannot accidentally
# inspect the later common-tree cwd instead of the actual KSU staging tree.
for rej in "${EXPECTED_SUKISU_REJECTS[@]}"; do
  if [ -f "$KSU_FOLDER/$rej" ]; then
    echo "Removing handled SukiSU/SUSFS reject: $rej"
    rm -f "$KSU_FOLDER/$rej"
  fi
done

if find "$KSU_FOLDER" -type f -name '*.rej' -print -quit | grep -q .; then
  echo "::error::Unexpected KernelSU-side .rej files remain after staging-tree compatibility repair:"
  find "$KSU_FOLDER" -type f -name '*.rej' -exec echo "=== {} ===" \; -exec cat {} \;
  exit 1
fi
echo "✅ No unresolved staging KernelSU .rej files remain"
