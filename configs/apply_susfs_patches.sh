...
# OP13R/A14/6.1 runtime diagnostics for SELinux-hide.
# This does not change hide semantics; it makes a failed hook installation
# visible in dmesg instead of looking like a successful Manager toggle.
inject_op61_selinux_hide_diagnostics() {
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then return 0; fi
  local target="$1"
  [ -f "$target" ] || return 0
  python3 - "$target" <<'PY_OP61_SELINUX_DIAG'
...
# Only inject once.
if 'OP61 selinux_hide: context_write patch' in s:
    raise SystemExit(0)

# Make each critical stage observable. Keep existing behavior untouched.
s = s.replace(
    'ret = ksu_patch_text(context_write, &my, sizeof(my), KSU_PATCH_TEXT_FLUSH_DCACHE);',
    'ret = ksu_patch_text(context_write, &my, sizeof(my), KSU_PATCH_TEXT_FLUSH_DCACHE);\\n'
    '    pr_info("KernelSU: OP61 selinux_hide: context_write patch ret=%d\\\\n", ret);',
    1)
s = s.replace(
    'ret = ksu_patch_text(access_write, &my, sizeof(my), KSU_PATCH_TEXT_FLUSH_DCACHE);',
    'ret = ksu_patch_text(access_write, &my, sizeof(my), KSU_PATCH_TEXT_FLUSH_DCACHE);\\n'
    '    pr_info("KernelSU: OP61 selinux_hide: access_write patch ret=%d\\\\n", ret);',
    1)
s = s.replace(
    'ret = ksu_lsm_hook(&selinux_setprocattr_hook);',
    'pr_info("KernelSU: OP61 selinux_hide: installing selinux_setprocattr hook\\\\n");\\n'
    '    ret = ksu_lsm_hook(&selinux_setprocattr_hook);\\n'
    '    pr_info("KernelSU: OP61 selinux_hide: selinux_setprocattr hook ret=%d\\\\n", ret);',
    1)
p.write_text(s)
print("OP61: injected SELinux-hide runtime diagnostics in", p)
PY_OP61_SELINUX_DIAG
}
...
static bool ksu_lsm_hook_target_matches(void *current_origin, void *target)
{
    ...
    /* Fast path: normal non-LTO registration. */
    if (current_origin == target)
        return true;

    /*
     * Android 14 / 6.1 builds use KCFI + LTO.  The function pointer stored
     * in security_hook_heads may point at an LTO-local alias/folded body
     * rather than the bare kallsyms address returned for target.  Match
     * aliases that live inside the resolved target's text range.
     */
    start = (unsigned long)target;
    current_addr = (unsigned long)current_origin;

    if (!kallsyms_lookup_size_offset(start, &size, NULL) || !size) {
        pr_warn("lsm_hook: unable to resolve target size for %pSb\n", target);
        return false;
    }

    if (current_addr >= start && current_addr < start + size) {
        pr_info("lsm_hook: KCFI/LTO alias %pSb matched target range %px-%px\n",
                current_origin, target, (void *)(start + size));
        return true;
    }
...
inject_op61_selinux_hide_diagnostics "kernel/feature/selinux_hide.c"
...
# Keep the source-side KernelSU tree compatible too.  The common/drivers mirror
# is normally the compiled copy, but newer manifests can switch which tree is
# consumed by Kbuild.  Applying the same deterministic fix to both prevents a
# silent regression when that layout changes.
if [ -f "$KSU_FOLDER/kernel/hook/lsm_hook.c" ]; then
  ensure_op61_lsm_hook_state "$KSU_FOLDER/kernel/hook/lsm_hook.c"
...
inject_op61_selinux_hide_diagnostics "drivers/kernelsu/feature/selinux_hide.c"
...
if [ "${ANDROID_VER_LOCAL:-}" = "android14" ] && [ "${KERNEL_VER_LOCAL:-}" = "6.1" ]; then
  for _lsm in "$KSU_FOLDER/kernel/hook/lsm_hook.c" "$COMMON_KERNEL_FOLDER/drivers/kernelsu/hook/lsm_hook.c"; do
    if [ -f "$_lsm" ]; then
      grep -q 'ksu_lsm_hook_target_matches' "$_lsm" || {
        echo "::error::OP 6.1 KCFI/LTO target-range matcher missing from $_lsm"
        exit 1
      }
    fi
  done
