...
# Clean SukiSU SUSFS integration mode.
# When the selected SukiSU tree already carries SUSFS (the supported non-GKI
# builtin/SUSFS integration), do NOT apply susfs4ksu's legacy
# 10_enable_susfs_for_ksu.patch on top of it. That patch targets a different
# KernelSU layout and was the source of the repeated sucompat/reject/linker
# failures seen on OP13R.
SUKISU_NATIVE_SUSFS=false
if [ -f "$KSU_FOLDER/kernel/Kconfig" ] &&    grep -qE '^[[:space:]]*config[[:space:]]+KSU_SUSFS([[:space:]]|$)' "$KSU_FOLDER/kernel/Kconfig" &&    [ -f "$KSU_FOLDER/kernel/feature/sucompat.c" ] &&    grep -q 'ksu_handle_stat' "$KSU_FOLDER/kernel/feature/sucompat.c" &&    grep -q 'ksu_handle_faccessat' "$KSU_FOLDER/kernel/feature/sucompat.c"; then
  SUKISU_NATIVE_SUSFS=true
  echo "✅ Native SukiSU SUSFS integration detected; legacy 10_enable_susfs_for_ksu.patch will be skipped."
fi

export SUKISU_NATIVE_SUSFS
...
validate_sucompat_object_built() {
  ...
  [ -z "$missing" ] || { echo "::error::Missing sucompat implementation(s):$missing in $c"; exit 1; }
...
# =============================================================================
# Clean native-SUSFS path for OP13R Android 14 / Linux 6.1
# =============================================================================
# SukiSU's native SUSFS integration must not be followed by the legacy
# susfs4ksu 10_enable_susfs_for_ksu.patch or its compatibility repair layer.
# The old path is what produced the partial sucompat/Kbuild integration and
# the final ksu_handle_* linker failures.
#
# The native path below applies only the kernel-side SUSFS patch, preserves
# the shared backup_sepolicy fix, enables CONFIG_KSU_SUSFS, and then exits.
# Other devices/trees continue through the legacy path below.
if [ "$SUKISU_NATIVE_SUSFS" = "true" ] && \
   [ "$ANDROID_VER_LOCAL" = "android14" ] && \
   [ "$KERNEL_VER_LOCAL" = "6.1" ] && \
   echo "$OP_MODEL" | grep -qiE '13r|oneplus[ _-]*13r'; then

  echo "=== Clean native-SUSFS OP13R 6.1 path ==="

  # Apply the kernel-side SUSFS patch only. Native SukiSU already owns the
  # KernelSU-side SUSFS integration.
  ...
  echo "Applying kernel-side SUSFS patch: $SUSFS_PATCH"
  if ! patch -p1 --forward < "$SUSFS_PATCH"; then
    echo "::error::Kernel-side SUSFS patch did not apply cleanly on OP13R 6.1."
    find . -name '*.rej' -print -exec sh -c 'echo "=== $1 ==="; cat "$1"' _ {} \;
    exit 1
  fi

  # Restore the exact vendor-compatible 3-argument set_nameidata form when
  # the OnePlus 6.1 tree uses it.
  if [ -f fs/namei.c ] && grep -q 'set_nameidata(nd, old_dfd, fake_filename, NULL)' fs/namei.c; then
    ...
    if [ "$nd_commas" -eq 2 ]; then
      sed -i 's/set_nameidata(nd, old_dfd, fake_filename, NULL)/set_nameidata(nd, old_dfd, fake_filename)/g' fs/namei.c
      echo "Dropped SUSFS OPEN_REDIRECT root argument for 3-arg vendor set_nameidata()"
    fi
  fi

  # Keep the already-identified SELinux backup fix; do not touch sucompat.
  ...
  if [ -f "$DEFCONFIG" ]; then
    ...
    echo "CONFIG_KSU_SUSFS=y enabled in gki_defconfig"
  fi

  # Native SukiSU must own its own Kbuild. Explicitly verify that the three
  # SUSFS VFS callers have matching implementations before compilation.
  FINAL_SUCOMPAT="$COMMON_KERNEL_FOLDER/drivers/kernelsu/feature/sucompat.c"
  FINAL_KBUILD="$COMMON_KERNEL_FOLDER/drivers/kernelsu/Kbuild"
  for fn in ksu_handle_stat ksu_handle_faccessat ksu_handle_post_execveat_sucompat; do
    if ! grep -qE "(^|[[:space:]])${fn}[[:space:]]*\(" "$FINAL_SUCOMPAT"; then
      echo "::error::Native SukiSU SUSFS is missing $fn in $FINAL_SUCOMPAT"
      exit 1
    fi
  done
  if ! grep -qE '(^|[[:space:]])kernelsu-objs[[:space:]]*\+=' "$FINAL_KBUILD" ||
     ! grep -q 'feature/sucompat\.o' "$FINAL_KBUILD"; then
    echo "::error::Native SukiSU Kbuild does not include feature/sucompat.o"
    exit 1
  fi

  echo "✅ Native SukiSU SUSFS integration verified for OP13R 6.1"
  echo "SUKISU_NATIVE_SUSFS=true" >> "${GITHUB_ENV}"
