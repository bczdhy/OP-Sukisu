...
ENABLE_KSU_PATCH="$SUSFS_FOLDER/kernel_patches/KernelSU/10_enable_susfs_for_ksu.patch"
ENABLE_KSU_LOG="/tmp/susfs_ksu_enable_patch.log"

echo "Applying SUSFS KernelSU enable patch with compatibility fuzz..."
if ! patch -p1 --forward -F 3 < "$ENABLE_KSU_PATCH" 2>&1 | tee "$ENABLE_KSU_LOG"; then
  echo "⚠️ SUSFS KernelSU enable patch reported rejects; attempting git 3-way fallback..."
  git apply --3way --reject --whitespace=nowarn "$ENABLE_KSU_PATCH" 2>&1 | tee -a "$ENABLE_KSU_LOG" || true
fi
...
if [ -n "$(find . -name '*.rej' -print -quit)" ]; then
  echo "SUSFS KernelSU enable patch produced reject files; they will be validated after SukiSU compatibility repair:"
...
# Do not fail on expected SukiSU/SUSFS rejects yet. The compatibility functions below
# intentionally repair the corresponding API drift (especially sucompat/stat hooks).
...
# The SUSFS enable patch may carry an older sucompat/fstat hunk. Do not silently
# discard that reject: the resulting source must expose the SukiSU vfs-fstat API.
verify_sukisu_fstat_api() {
  local base="$1"
  local c="$base/feature/sucompat.c"
  [ -f "$c" ] || return 0

  if grep -qE 'ksu_sys_fstat[[:space:]]*\(' "$c" && ! grep -q 'ksu_handle_vfs_fstat' "$c"; then
    echo "::error::SukiSU sucompat still contains the old ksu_sys_fstat wrapper in $c"
    echo "::error::The SUSFS fstat hunk did not apply; refusing to delete sucompat.c.rej"
    grep -nE 'ksu_sys_fstat|ksu_handle_vfs_fstat|orig_sys_fstat' "$c" || true
    return 1
  fi
...
if ! verify_sukisu_fstat_api "kernel"; then
  echo "::error::KernelSU fstat compatibility is unresolved."
  exit 1
fi

# Now that compatibility repair has completed, remove only the expected reject files.
