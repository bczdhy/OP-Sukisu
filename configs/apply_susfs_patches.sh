...
HEOF
  ...
  if [ -f "$ksud_integration_c" ]; then
    ...
    # instead of fabricating invalid NULL kernel-pointer arguments.
    sed -i '/ksu_handle_execveat_ksud(path, *\\&argv);/d' "$ksud_integration_c"
    ...
    # legacy syscall-hook cleanup. Restore them at function entry if missing.
    python3 - "$ksud_integration_c" <<'PYFSTAT'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()
pat = r'(void\s+ksu_handle_vfs_fstat\s*\(\s*int\s+fd\s*,\s*loff_t\s*\*\s*kstat_size_ptr\s*\)\s*\{)(?!\s*loff_t\s+orig_size\s*=)'
