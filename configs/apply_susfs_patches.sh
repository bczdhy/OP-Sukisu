...
apply_known_susfs_reject() {
  ...
  # SukiSU may have already rewritten nearby lines (notably adb_root.c),
  # leaving the exact SUSFS hunk with insufficient context. Retry the exact
  # reject with bounded GNU-patch fuzz; never synthesize replacement code.
  if [ "$rej" = "kernel/feature/adb_root.c.rej" ]; then
    for fuzz in 3 5 7; do
      if patch -p0 --forward --batch --fuzz="$fuzz" < "$rej"; then
        echo "  ✅ Applied rejected SUSFS adb_root hunk with fuzz=$fuzz: $rej"
        rm -f "$rej"
        return 0
      fi
    done
