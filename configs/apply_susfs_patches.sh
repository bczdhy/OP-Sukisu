...
def function_span(text, signature_re):
    m = re.search(signature_re, text, re.M)
    if not m:
        return None
    start = m.start()
...
old_do_re = r"static\s+long\s+do_ksu_adb_root_handle_execve\s*\(\s*const\s+char\s+__user\s*\*\s*filename_user\s*,\s*struct\s+pt_regs\s*\*\s*regs\s*,\s*unsigned\s+long\s*\*\s*envp_p\s*\)"
...
if re.search(r"\blong\s+ksu_adb_root_handle_execveat\s*\(\s*const\s+char\s*\*\s*filename\s*,\s*void\s*\*\*\*\s*envp_user_ptr\s*\)", s):
    raise SystemExit(3)

old_do = function_span(s, old_do_re)
old_exec = function_span(s, old_exec_re)
old_execat = function_span(s, old_execat_re)
if not (old_do and old_exec and old_execat):
    raise SystemExit(2)
...
fn = re.sub(old_do_re,
    ...
    fn, count=1)
fn = fn.replace("is_exec_adbd(filename_user)", "is_exec_adbd(filename)", 1)
fn = fn.replace("setup_ld_preload(regs, envp_p)", "setup_ld_preload(envp_user_ptr)", 1)
fn = fn.replace("user_stack_pointer(regs)", "current_user_stack_pointer()", 1)
fn = re.sub(r"unsigned\s+long\s*\*\s*envp_p\s*=\s*\(unsigned\s+long\s*\*\)\s*&?PT_REGS_PARM3\(regs\)\s*;",
            "unsigned long *envp_p = (unsigned long *)envp_user_ptr;", fn, count=1)
s = s[:a] + fn + s[b:]
