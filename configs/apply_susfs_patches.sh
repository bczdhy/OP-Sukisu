...
ensure_backup_sepolicy_symbol() {
  ...
  local rules="$root/drivers/kernelsu/selinux/rules.c"
  local sepolicy_h="$root/drivers/kernelsu/selinux/sepolicy.h"
  local hide="$root/drivers/kernelsu/feature/selinux_hide.c"

  [ -f "$hide" ] || return 0

  # KernelSU's backup policy is a single shared object.  rules.c owns the
  # storage; selinux_hide.c only consumes it.  Never create a second definition
  # inside selinux_hide.c because that gives the feature its own unrelated copy.
  python3 - "$rules" "$sepolicy_h" "$hide" <<'PY_BACKUP_SEPOLICY'
from pathlib import Path
import re
import sys

rules, header, hide = map(Path, sys.argv[1:])

# 1. Remove the bad definition previously injected into selinux_hide.c.
if hide.exists():
    s = hide.read_text()
    s = re.sub(
...
# 2. rules.c must contain exactly one real definition.
def_re = re.compile(
    r'(?m)^\s*struct\s+selinux_policy\s*\*\s*backup_sepolicy\s*;\s*$'
)

defs = []
for base in [rules.parent.parent.parent, rules.parent.parent.parent.parent]:
    if not base.exists():
        continue
    for p in base.rglob('rules.c'):
        try: t = p.read_text()
        except Exception: continue
        if def_re.search(t):
            defs.append(p)

# Prefer the expected drivers/kernelsu/selinux/rules.c location.
if rules.exists():
    if not def_re.search(rules.read_text()):
        rs = rules.read_text()
        # Put it immediately after the local include block, before code.
        marker = '#include "xfrm.h"'
        line = '\n\n/* Shared clean SELinux policy snapshot used by selinux_hide. */\nstruct selinux_policy *backup_sepolicy;\n'
        if marker in rs:
            rs = rs.replace(marker, marker + line, 1)
        else:
            rs = line.lstrip('\n') + rs
        rules.write_text(rs)

# 3. Export the declaration from sepolicy.h, which selinux_hide.c already sees.
if header.exists():
    hs = header.read_text()
    extern = 'extern struct selinux_policy *backup_sepolicy;'
    # Remove stale duplicate declarations first, then add exactly one.
    hs = re.sub(r'(?m)^\s*extern\s+struct\s+selinux_policy\s*\*\s*backup_sepolicy\s*;\s*\n?', '', hs)
    if extern not in hs:
        anchor = 'struct selinux_policy *ksu_dup_sepolicy(struct selinux_policy *old_pol);'
        if anchor in hs:
            hs = hs.replace(anchor, anchor + '\n' + extern, 1)
        else:
            hs = hs.replace('#define __KSU_H_SEPOLICY\n', '#define __KSU_H_SEPOLICY\n\n' + extern + '\n', 1)
    header.write_text(hs)

# 4. Validate exactly one C definition under the KSU trees.
all_defs = []
for base in [rules.parent.parent.parent, rules.parent.parent.parent.parent]:
    if not base.exists():
        continue
    for p in base.rglob('*.c'):
        ...
        if def_re.search(t):
            all_defs.append(p)
if len(all_defs) != 1:
    raise SystemExit(f'backup_sepolicy validation failed: expected 1 definition, found {all_defs}')
if header.exists() and 'extern struct selinux_policy *backup_sepolicy;' not in header.read_text():
    raise SystemExit(f'backup_sepolicy validation failed: missing extern in {header}')

print(f'  [SELinux-hide] shared backup_sepolicy definition: {all_defs[0]}')
print(f'  [SELinux-hide] shared backup_sepolicy declaration: {header}')
PY_BACKUP_SEPOLICY
}

# The SUSFS syscall/init-rc variant can leave these two forward declarations
# unused in built-in configurations.  They are declarations only, so removing
# them has no ABI/runtime effect and keeps -Wunused-variable clean.
fix_unused_initrc_syscall_decls() {
  local root="$1"
  local target="$root/drivers/kernelsu/runtime/ksud_integration.c"
  [ -f "$target" ] || return 0
  sed -i \
    '/^[[:space:]]*static long (\*orig_sys_read)(const struct pt_regs \*regs);[[:space:]]*$/d' \
    -e '/^[[:space:]]*static long (\*orig_sys_fstat)(const struct pt_regs \*regs);[[:space:]]*$/d' \
    "$target" || true
}
...
fix_unused_initrc_syscall_decls "$KSU_FOLDER"
fix_unused_initrc_syscall_decls "$COMMON_KERNEL_FOLDER"
