...
if [ -f kernel/feature/adb_root.c.rej ]; then
  ...
  # The SUSFS v2.2.x adb_root hunk is version-specific. Some SukiSU trees
  # have already removed both sides of that hunk: they contain neither the
  # old ksud execve hooks nor the newer ksu_handle_vfs_fstat() helper.
  #
  # In that case the .rej is an obsolete patch fragment, not an indication
  # that either implementation is missing. Only discard it when the reject
  # itself is demonstrably the old execve->vfs_fstat replacement hunk.
  # Match the reject using fixed-string probes as well as the older ERE.  The
  # reject is emitted by git/patch and may contain context/formatting that makes
  # the compound ERE brittle, but these SUSFS v2.3.0 signatures are unambiguous.
  # Detect the SUSFS adb_root ABI hunk by individual, fixed SUSFS signatures.
  # Do not use a compound ERE here: patch rejects contain +/- lines and
  # toolchain grep variants can make the old expression miss a valid reject.
  if grep -Fq 'setup_ld_preload(envp_user_ptr)' kernel/feature/adb_root.c.rej || \
     grep -Fq 'escape_with_root_profile() failed' kernel/feature/adb_root.c.rej || \
     grep -Fq 'long ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)' kernel/feature/adb_root.c.rej || \
     grep -Fq 'do_ksu_adb_root_handle_execveat(filename, envp_user_ptr)' kernel/feature/adb_root.c.rej || \
     grep -Fq 'ksu_handle_vfs_fstat' kernel/feature/adb_root.c.rej || \
     grep -Fq 'ksu_execve_hook_ksud' kernel/feature/adb_root.c.rej || \
     grep -Fq 'ksu_execveat_hook_ksud' kernel/feature/adb_root.c.rej; then

    # SUSFS v2.3.0 changes adb-root from the old pt_regs syscall ABI to the
    # VFS execveat ABI. On SukiSU 409xx the preceding adb_root hunks can apply
    # while this final function-boundary hunk rejects. Reconcile it instead of
    # silently dropping the reject, because adb_root.h has already changed ABI.
    if grep -qE '^[[:space:]]*long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const[[:space:]]+char[[:space:]]*\*filename' "$target" && \
       grep -qE 'do_ksu_adb_root_handle_execveat' "$target"; then
      echo "  adb_root.c: SUSFS execveat ABI already present; reject is obsolete"
      rm -f kernel/feature/adb_root.c.rej

    elif grep -qE '^[[:space:]]*static[[:space:]]+long[[:space:]]+do_ksu_adb_root_handle_execve\(' "$target"; then
      echo "  adb_root.c: applying SUSFS v2.3.0 execve -> execveat compatibility hunk"

      # Earlier SUSFS hunks have already converted is_exec_adbd() and
      # setup_ld_preload(). Convert the remaining function boundary and wrappers.
      perl -0pi -e 's/static long do_ksu_adb_root_handle_execve\(const char __user \*filename_user, struct pt_regs \*regs, unsigned long \*envp_p\)/static long do_ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)/g' "$target"
      perl -0pi -e 's/is_exec_adbd\(filename_user\)/is_exec_adbd(filename)/g; s/setup_ld_preload\(regs, envp_p\)/setup_ld_preload(envp_user_ptr)/g' "$target"

      # The v2.3.0 patch removes the old execve wrapper and routes execveat
      # directly to the converted internal handler.
      ...
      # SUSFS v2.3.0 applies the selected root profile after adb-root setup.
      if ! grep -q 'escape_with_root_profile() failed' "$target"; then
        perl -0pi -e 's/(escape_to_root_for_adb_root\(\);\n)(\s*return 0;\n\})/$1\n    ret = escape_with_root_profile();\n    if (ret)\n        pr_err("escape_with_root_profile() failed: %d\\n", (int)ret);\n\n$2/s' "$target"
      fi

      if grep -qE 'ksu_adb_root_handle_execve\(struct pt_regs' "$target"; then
        echo "::error::Legacy ksu_adb_root_handle_execve wrapper remains after SUSFS ABI repair"
        exit 1
      fi
      if ! grep -qE 'long ksu_adb_root_handle_execveat\(const char \*filename, void \*\*\*envp_user_ptr\)' "$target"; then
        echo "::error::SUSFS execveat wrapper was not installed"
        exit 1
      fi
      rm -f kernel/feature/adb_root.c.rej

    elif grep -qE '^[[:space:]]*void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\(' "$target"; then
      echo "  adb_root.c: newer ksu_handle_vfs_fstat API already present; reject is obsolete"
      rm -f kernel/feature/adb_root.c.rej

    else
      echo "::error::adb_root.c SUSFS reject needs manual reconciliation; refusing to discard it"
      cat kernel/feature/adb_root.c.rej
      exit 1
    fi

  else
    # Final safety probe for the exact SUSFS v2.3.0 adb_root ABI reject.
    # This intentionally accepts only the known function-boundary conversion
    # shown by the SUSFS patch; unrelated rejects still fail safely.
    if grep -Fq 'setup_ld_preload(envp_user_ptr)' kernel/feature/adb_root.c.rej && \
       grep -Fq 'do_ksu_adb_root_handle_execveat(filename, envp_user_ptr)' kernel/feature/adb_root.c.rej; then
      echo "  adb_root.c: recognized SUSFS v2.3.0 execve -> execveat reject"
      if grep -Fq 'long ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)' "$target" && \
         grep -Fq 'do_ksu_adb_root_handle_execveat(filename, envp_user_ptr)' "$target"; then
        rm -f kernel/feature/adb_root.c.rej
      else
        echo "::error::SUSFS adb_root reject recognized, but target ABI was not reconciled"
        cat kernel/feature/adb_root.c.rej
        exit 1
      fi
    else
