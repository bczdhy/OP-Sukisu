...
if [ -f kernel/feature/adb_root.c.rej ]; then
  ...
  if grep -qE 'ksu_handle_vfs_fstat|ksu_execve_hook_ksud|ksu_execveat_hook_ksud|ksu_execve_hook_ksud_common|do_ksu_adb_root_handle_execveat|setup_ld_preload\(envp_user_ptr\)' \
      kernel/feature/adb_root.c.rej; then
