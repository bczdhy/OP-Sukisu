...
# Match the function by name rather than assuming one exact parameter spelling.
# SukiSU revisions use slightly different fstat parameter types.
pat = r'(\b(?:static\s+)?(?:long|int|void)\s+ksu_handle_vfs_fstat\s*\([^{};]*\)\s*\{)'
m = re.search(pat, s, flags=re.S)
if m:
    body_start = m.end()
    next_chunk = s[body_start:body_start + 500]
    if not re.search(r'\bloff_t\s+orig_size\s*=\s*\*kstat_size_ptr\s*;', next_chunk):
        # Only add the locals when this function actually exposes kstat_size_ptr.
        if re.search(r'\bkstat_size_ptr\b', s[body_start:s.find('\n}', body_start) if s.find('\n}', body_start) != -1 else body_start + 5000]):
            s = s[:body_start] + '\n    loff_t orig_size = *kstat_size_ptr;\n    size_t extra = 0;' + s[body_start:]
            p.write_text(s)
else:
