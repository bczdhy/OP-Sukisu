...
ensure_op61_selinux_hide_init() {
  ...
  # OP 6.1's older lsm_hook.c does not expose the newer lifecycle entry points
  # ksu_lsm_hook_init()/ksu_lsm_hook_exit(). Do not inject those calls: they create
  # undefined linker symbols. selinux_hide_init() directly registers the hooks via
  # ksu_lsm_hook(), which is the functionality we actually need here.
  grep -q '#include "feature/selinux_hide.h"' "$target" || \
    sed -i '/#include "ksu.h"/a #include "feature/selinux_hide.h"' "$target"

  if ! grep -RqsE '^[[:space:]]*(static[[:space:]]+)?void[[:space:]]+(__init[[:space:]]+)?ksu_selinux_hide_init[[:space:]]*\(' "$root_dir" --include='*.c' 2>/dev/null; then
    echo "::error::ksu_selinux_hide_init() definition is missing from the OP 6.1 KSU tree"
    exit 1
  fi

  if ! grep -qE '^[[:space:]]*ksu_selinux_hide_init[[:space:]]*\(\);' "$target"; then
    if grep -qE '^[[:space:]]*ksu_adb_root_init[[:space:]]*\(\);' "$target"; then
      sed -i '/^[[:space:]]*ksu_adb_root_init[[:space:]]*();/a\    ksu_selinux_hide_init();' "$target"
    elif grep -qE '^[[:space:]]*ksu_feature_init[[:space:]]*\(\);' "$target"; then
      sed -i '/^[[:space:]]*ksu_feature_init[[:space:]]*();/a\    ksu_selinux_hide_init();' "$target"
    else
      echo "::error::No safe init anchor for ksu_selinux_hide_init() in $target"
      exit 1
    fi
  fi

  # If an earlier patch inserted the unsupported lsm lifecycle calls, remove them.
  sed -i '/^[[:space:]]*ksu_lsm_hook_init[[:space:]]*();[[:space:]]*$/d' "$target" || true
  sed -i '/^[[:space:]]*ksu_lsm_hook_exit[[:space:]]*();[[:space:]]*$/d' "$target" || true

  grep -qE '^[[:space:]]*ksu_selinux_hide_init[[:space:]]*\(\);' "$target" || {
    echo "::error::OP 6.1 missing ksu_selinux_hide_init()"
    exit 1
  }
  echo "✅ OP 6.1 SELinux-hide init wired without unsupported lsm lifecycle calls"
}
