...
fix_sukisu_adb_root_api() {
  ...
  if grep -qE 'long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const char \*filename[[:space:]]*,[[:space:]]*void \*\*\*envp_user_ptr' "$target" &&
     grep -qE 'static long[[:space:]]+do_ksu_adb_root_handle_execveat[[:space:]]*\(' "$target"; then
    echo "  newer adb_root API already present"; rm -f "$rej"; return 0
  fi
  if ! grep -qE 'ksu_handle_vfs_fstat|ksu_adb_root_handle_execveat|setup_ld_preload|do_ksu_adb_root_handle_execve' "$rej"; then
    echo "::error::Unexpected adb_root.c reject content"; cat "$rej"; exit 1
  fi
  python3 - "$target" <<'PY2'
from pathlib import Path
import re, sys
p=Path(sys.argv[1]); s=p.read_text()
new_block='''static long do_ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)
{
...
'''
pat=re.compile(r'static long do_ksu_adb_root_handle_execve\s*\([^{}]*\{.*?\n\}\s*\nlong ksu_adb_root_handle_execve\s*\([^{}]*\{.*?\n\}\s*\nlong ksu_adb_root_handle_execveat\s*\([^{}]*\{.*?\n\}\s*',re.S)
m=pat.search(s)
if not m: raise SystemExit('Could not locate legacy adb_root execve/execveat block')
p.write_text(s[:m.start()]+new_block+s[m.end():])
PY2
  if grep -qE 'long[[:space:]]+ksu_adb_root_handle_execve[[:space:]]*\(' "$target"; then
    echo "::error::Legacy ksu_adb_root_handle_execve() remains in $target"; exit 1
  fi
  if ! grep -qE 'long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const char \*filename[[:space:]]*,[[:space:]]*void \*\*\*envp_user_ptr' "$target"; then
    echo "::error::New ksu_adb_root_handle_execveat() API missing in $target"; exit 1
  fi
  rm -f "$rej"
