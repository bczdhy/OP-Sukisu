...
# =============================================================================
# SukiSU compatibility for SUSFS v2.3.x adb_root/supercall API migration
#
# SUSFS 60024a4... expects the newer inline-hook APIs, while the pinned
# SukiSU cf87e3f4... tree still has the older pt_regs/kprobe entry points.
# The enable patch therefore leaves adb_root.c.rej and supercall.c.rej.
# Convert those two known API boundaries explicitly and validate the result.
# =============================================================================

fix_sukisu_adb_root_api() {
  local target="kernel/feature/adb_root.c"
  local rej="kernel/feature/adb_root.c.rej"
  [ -f "$target" ] || return 0
  [ -f "$rej" ] || return 0

  echo "Fixing SukiSU adb_root API compatibility in: $target"

  # This reject is the recent SUSFS migration from the old pt_regs execve
  # entry point to the filename/envp_user_ptr API. Validate the reject before
  # touching the source; never discard an unrelated reject.
  if ! grep -qE 'setup_ld_preload\(envp_user_ptr\)|ksu_adb_root_handle_execveat|do_ksu_adb_root_handle_execveat|escape_with_root_profile' "$rej"; then
    echo "::error::Unexpected adb_root.c reject content"
    cat "$rej"
    exit 1
  fi
...
# Required includes from the current SUSFS adb_root implementation.
for inc in (
    "#include <linux/sched/task_stack.h>",
    "#include <linux/susfs_def.h>",
):
    if inc not in s:
        marker = "#include <linux/slab.h>"
        if marker in s:
            s = s.replace(marker, marker + "\n" + inc, 1)
        else:
            s = inc + "\n" + s

if '#include "policy/app_profile.h"' not in s:
    marker = '#include "arch.h"'
    if marker in s:
        s = s.replace(marker, marker + '\n#include "policy/app_profile.h"', 1)
    else:
        s = '#include "policy/app_profile.h"\n' + s

# Convert is_exec_adbd() to consume the resolved filename.
new_is_exec = """static inline long is_exec_adbd(const char *filename)
{
...
m = re.search(r'static\s+(?:inline\s+)?long\s+is_exec_adbd\s*\([^)]*\)\s*\{.*?\n\}', s, re.S)
if not m:
    raise SystemExit("Could not locate is_exec_adbd()")
s = s[:m.start()] + new_is_exec + s[m.end():]

# Convert setup_ld_preload().
# SukiSU trees in the wild have two legacy setup_ld_preload() shapes:
#   1) setup_ld_preload(struct pt_regs *regs, unsigned long *envp_p)
#   2) setup_ld_preload(struct pt_regs *regs)
# The SUSFS v2.3 patch wants the common filename/envp_user_ptr API.
sig_patterns = [
    r'static\s+long\s+setup_ld_preload\s*\(\s*(?:const\s+)?struct\s+pt_regs\s*\*\s*regs\s*,\s*unsigned\s+long\s*\*\s*envp_p\s*\)',
    r'static\s+long\s+setup_ld_preload\s*\(\s*(?:const\s+)?struct\s+pt_regs\s*\*\s*regs\s*\)',
    r'long\s+setup_ld_preload\s*\(\s*(?:const\s+)?struct\s+pt_regs\s*\*\s*regs\s*,\s*unsigned\s+long\s*\*\s*envp_p\s*\)',
    r'long\s+setup_ld_preload\s*\(\s*(?:const\s+)?struct\s+pt_regs\s*\*\s*regs\s*\)',
]
n1 = 0
for pat in sig_patterns:
    s, n = re.subn(pat, 'static long setup_ld_preload(void ***envp_user_ptr)', s, count=1)
    n1 += n
    if n:
        break
if n1 != 1:
    raise SystemExit("Could not convert setup_ld_preload() signature")

# Normalize the legacy stack-pointer access. SukiSU revisions use several
# equivalent spellings here (user_stack_pointer(regs), user_stack_pointer(),
# PT_REGS-derived expressions, or slightly different whitespace). Limit the
# replacement to the stackp declaration so unrelated user_stack_pointer()
# uses elsewhere in adb_root.c are untouched.
stackp_patterns = [
    r'unsigned\s+long\s+stackp\s*=\s*user_stack_pointer\s*\([^;]*\)\s*;',
    r'unsigned\s+long\s+stackp\s*=\s*PT_REGS_[A-Z0-9_]+\s*\([^;]*\)\s*;',
]
n2 = 0
for pat in stackp_patterns:
    s, n = re.subn(
        pat,
...
# Some trees already use the modern helper; that is already the desired state.
if n2 == 0 and not re.search(
    r'unsigned\s+long\s+stackp\s*=\s*current_user_stack_pointer\s*\(\s*\)\s*;',
    s,
):
    raise SystemExit("Could not convert user_stack_pointer()")

# Normalize either the old explicit envp_p assignment or the variant where
# setup_ld_preload() received envp_p as its second argument.
envp_patterns = [
    r'unsigned\s+long\s*\*\s*envp_p\w*\s*=\s*\(unsigned\s+long\s*\*\)\s*&?\s*PT_REGS_PARM3\s*\(\s*regs\s*\)\s*;',
    r'unsigned\s+long\s*\*\s*envp_p\w*\s*=\s*\(unsigned\s+long\s*\*\)\s*PT_REGS_SYSCALL_PARM4\s*\(\s*regs\s*\)\s*;',
]
n3 = 0
for pat in envp_patterns:
    s, n = re.subn(pat, 'unsigned long *envp_p = (unsigned long *)envp_user_ptr;', s, count=1)
    n3 += n
    if n:
        break

# If this legacy variant had no envp_p declaration at all, add the normalized
# declaration immediately after stackp inside setup_ld_preload().
if not re.search(r'unsigned\s+long\s*\*\s*envp_p\s*=\s*\(unsigned\s+long\s*\*\)\s*envp_user_ptr\s*;', s):
    setup_start = s.find('static long setup_ld_preload(void ***envp_user_ptr)')
    if setup_start < 0:
        raise SystemExit("Converted setup_ld_preload() function was not found")
    stack_stmt = re.search(
        r'unsigned\s+long\s+stackp\s*=\s*current_user_stack_pointer\s*\(\s*\)\s*;',
        s[setup_start:],
    )
    if not stack_stmt:
        raise SystemExit("Could not locate normalized stack pointer inside setup_ld_preload()")
    pos = setup_start + stack_stmt.end()
    s = s[:pos] + '\n    unsigned long *envp_p = (unsigned long *)envp_user_ptr;' + s[pos:]

s = s.replace("adb_root_handle_execve failed", "adb_root_handle_execveat failed")

# Convert the do_* helper.
s, n4 = re.subn(
    r'static\s+long\s+do_ksu_adb_root_handle_execve\s*\(\s*const\s+char\s+__user\s*\*\s*filename_user\s*,\s*struct\s+pt_regs\s*\*\s*regs\s*,\s*unsigned\s+long\s*\*\s*envp_p\s*\)',
    'static long do_ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)',
    s, count=1)
if n4 != 1:
    raise SystemExit("Could not convert do_ksu_adb_root_handle_execve() signature")

s = s.replace("is_exec_adbd(filename_user)", "is_exec_adbd(filename)")

s, n5 = re.subn(
    r'long\s+ret\s*=\s*setup_ld_preload\s*\(\s*regs\s*,\s*envp_p\s*\)\s*;',
    'long ret = setup_ld_preload(envp_user_ptr);', s, count=1)
if n5 != 1:
    raise SystemExit("Could not convert setup_ld_preload() call")

# Recent SUSFS requires the root profile transition here.
if "escape_with_root_profile();" not in s:
    needle = "    escape_to_root_for_adb_root();"
    if needle not in s:
        raise SystemExit("Could not locate escape_to_root_for_adb_root()")
    s = s.replace(
        needle,
        needle + '\n'
...
# Remove the old execve wrapper completely.
old = re.search(
    r'\nlong\s+ksu_adb_root_handle_execve\s*\(\s*struct\s+pt_regs\s*\*\s*regs\s*\)\s*\{.*?\n\}\n',
    s, re.S)
if old:
    s = s[:old.start()] + "\n" + s[old.end():]
elif re.search(r'\blong\s+ksu_adb_root_handle_execve\s*\(', s):
    raise SystemExit("Could not safely remove legacy ksu_adb_root_handle_execve()")

# Change execveat wrapper signature.
s, n6 = re.subn(
    r'\blong\s+ksu_adb_root_handle_execveat\s*\(\s*struct\s+pt_regs\s*\*\s*regs\s*\)',
    'long ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)',
    s, count=1)
if n6 != 1:
    raise SystemExit("Could not convert ksu_adb_root_handle_execveat() signature")

# Replace its old do_* call.
s, n7 = re.subn(
    r'return\s+do_ksu_adb_root_handle_execve\s*\(\s*'
    r'\(const\s+char\s+__user\s*\*\)\s*PT_REGS_PARM2\s*\(\s*regs\s*\)\s*,\s*'
    r'regs\s*,\s*\(unsigned\s+long\s*\*\)\s*&\s*PT_REGS_SYSCALL_PARM4\s*\(\s*regs\s*\)\s*\)\s*;',
    'return do_ksu_adb_root_handle_execveat(filename, envp_user_ptr);',
    s, count=1, flags=re.S)
if n7 != 1:
    raise SystemExit("Could not replace legacy execveat call")

p.write_text(s)
PY2

  # Keep adb_root.h synchronized with the migrated implementation. The SUSFS
  # patch normally updates this file itself, but make the compatibility helper
  # idempotent for trees where that hunk also rejected.
  if [ -f kernel/feature/adb_root.h ]; then
    sed -i '/^[[:space:]]*long[[:space:]]\+ksu_adb_root_handle_execve(struct pt_regs \*regs);[[:space:]]*$/d' kernel/feature/adb_root.h
    sed -i '/^[[:space:]]*long[[:space:]]\+ksu_adb_root_handle_execveat(struct pt_regs \*regs);[[:space:]]*$/d' kernel/feature/adb_root.h
    grep -q 'long ksu_adb_root_handle_execveat(const char \*filename, void __user \*\*\*envp_user_ptr);' kernel/feature/adb_root.h || \
      printf '\nlong ksu_adb_root_handle_execveat(const char *filename, void __user ***envp_user_ptr);\n' >> kernel/feature/adb_root.h
  fi

  # Verify before discarding the reject.
  if grep -qE 'long[[:space:]]+ksu_adb_root_handle_execve[[:space:]]*\([[:space:]]*struct[[:space:]]+pt_regs' "$target"; then
    echo "::error::Legacy ksu_adb_root_handle_execve(pt_regs) remains in $target"
    exit 1
  fi

  grep -qE 'long[[:space:]]+ksu_adb_root_handle_execveat[[:space:]]*\([[:space:]]*const[[:space:]]+char[[:space:]]+\*filename[[:space:]]*,[[:space:]]*void[[:space:]]*\*\*\*envp_user_ptr' "$target" || {
    echo "::error::New adb_root execveat API missing in $target"
    exit 1
  }

  grep -q 'setup_ld_preload(envp_user_ptr)' "$target" || {
    echo "::error::adb_root.c still uses the old setup_ld_preload() API"
    exit 1
  }

  grep -q 'current_user_stack_pointer()' "$target" || {
    echo "::error::adb_root.c still uses user_stack_pointer(regs)"
    exit 1
  }

  grep -q 'escape_with_root_profile()' "$target" || {
    echo "::error::adb_root.c is missing escape_with_root_profile()"
    exit 1
  }

  rm -f "$rej"
  echo "✅ Fixed SukiSU adb_root API in: $target"
}

fix_sukisu_supercall_reboot_api() {
  local target="kernel/supercall/supercall.c"
  local rej="kernel/supercall/supercall.c.rej"
  [ -f "$target" ] || return 0
  [ -f "$rej" ] || return 0
  echo "Fixing SukiSU supercall reboot API in: $target"
  if grep -qE '^[[:space:]]*int[[:space:]]+ksu_supercall_reboot_handler[[:space:]]*\(' "$target" &&
     ! grep -qE '^[[:space:]]*static int[[:space:]]+reboot_handler_pre[[:space:]]*\(' "$target"; then
    echo "  newer supercall reboot API already present"; rm -f "$rej"; return 0
  fi
  if ! grep -qE 'ksu_supercall_reboot_handler|reboot_handler_pre|register_kprobe|reboot_kp' "$rej"; then
    echo "::error::Unexpected supercall.c reject content"; cat "$rej"; exit 1
  fi
  python3 - "$target" <<'PY3'
from pathlib import Path
import re, sys
p=Path(sys.argv[1]); s=p.read_text()
new_handler='''int ksu_supercall_reboot_handler(void __user **arg)
{
...
'''
pat=re.compile(r'static int reboot_handler_pre\s*\([^{}]*\{.*?\n\}\s*\nstatic struct kprobe reboot_kp\s*=\s*\{.*?\n\};\s*',re.S)
m=pat.search(s)
if not m: raise SystemExit('Could not locate legacy reboot kprobe block')
s=s[:m.start()]+new_handler+s[m.end():]
s=re.sub(r'\n[ \t]*int rc;\n\n[ \t]*rc = register_kprobe\(&reboot_kp\);\n[ \t]*if \(rc\) \{\n[ \t]*pr_err\("reboot kprobe failed: %d\\n", rc\);\n[ \t]*\} else \{\n[ \t]*pr_info\("reboot kprobe registered successfully\\n"\);\n[ \t]*\}\n','\n',s,flags=re.S)
s=re.sub(r'\n[ \t]*unregister_kprobe\(&reboot_kp\);\n','\n',s)
p.write_text(s)
PY3
  if grep -qE '^[[:space:]]*static int[[:space:]]+reboot_handler_pre[[:space:]]*\(' "$target"; then
    echo "::error::Legacy reboot_handler_pre() remains in $target"; exit 1
  fi
  if grep -qE 'reboot_kp|register_kprobe\(&reboot_kp\)|unregister_kprobe\(&reboot_kp\)' "$target"; then
    echo "::error::Legacy reboot kprobe registration remains in $target"; exit 1
  fi
  if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_supercall_reboot_handler[[:space:]]*\(' "$target"; then
    echo "::error::ksu_supercall_reboot_handler() missing in $target"; exit 1
  fi
  rm -f "$rej"
  echo "✅ Fixed SukiSU supercall reboot API in: $target"
}
