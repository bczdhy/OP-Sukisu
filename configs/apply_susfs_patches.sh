...
ensure_op61_selinux_backup_init() {
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then return 0; fi
  local target="$1"
  [ -f "$target" ] || return 0
  echo "Ensuring OP 6.1 SELinux backup initialization in: $target"

  python3 - "$target" <<'PY_OP61_BACKUP_INIT'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()

# SukiSU's SELinux-hide backup is populated by apply_kernelsu_rules().
# On some Android 14 / 6.1 trees the second-stage init path reaches the
# SELinux-hide feature before that routine is called, leaving backup_sepolicy
# NULL and causing selinux_hide_enable() to return -EAGAIN.
# Upstream's second-stage path calls apply_kernelsu_rules() before cache_sid()
# and setup_ksu_cred().  Add that call immediately after the existing
# second-stage SELinux-hide initialization, but only when it is absent from
# the same second-stage block.
needle = 'ksu_selinux_hide_handle_second_stage();'
if needle not in s:
    print('OP61: second-stage SELinux-hide call not found in', p)
    sys.exit(0)

# Do not duplicate the call if the tree already has it nearby.
for m in re.finditer(re.escape(needle), s):
    tail = s[m.end():m.end()+500]
    if re.search(r'\bapply_kernelsu_rules\s*\(\s*\)\s*;', tail):
        print('OP61: apply_kernelsu_rules() already follows second-stage SELinux-hide call in', p)
        sys.exit(0)

s = s.replace(needle, needle + '\n\t\t\tapply_kernelsu_rules();', 1)
p.write_text(s)
print('OP61: inserted apply_kernelsu_rules() after second-stage SELinux-hide initialization in', p)
PY_OP61_BACKUP_INIT
}
...
# =============================================================================
# OP 6.1 SELinux-hide runtime diagnostics (temporary)
# =============================================================================
debug_op61_selinux_hide_runtime() {
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then return 0; fi
  for target in \
    "$KSU_FOLDER/kernel/feature/selinux_hide.c" \
    "$COMMON_KERNEL_FOLDER/drivers/kernelsu/feature/selinux_hide.c"; do
    [ -f "$target" ] || continue
    python3 - "$target" <<'PY_OP61_DIAG'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()
if 'OP61_SELINUX_HIDE_RUNTIME_DIAG' in s:
    print('OP61: runtime diagnostics already present in', p)
    continue_file = True
else:
    continue_file = False

if not continue_file:
    m = re.search(r'(\b(?:static\s+)?int\s+ksu_selinux_hide_enable\s*\([^)]*\)\s*\{)', s)
    if m:
        dbg = """
    /* OP61_SELINUX_HIDE_RUNTIME_DIAG */
    pr_err(\"KernelSU: OP61 selinux_hide DIAG: enable entered, backup_sepolicy=%px\\n\", backup_sepolicy);"""
        s = s[:m.end()] + dbg + s[m.end():]
    else:
        print('OP61: ksu_selinux_hide_enable() not found in', p)

    m = re.search(r'if\s*\(\s*!backup_sepolicy\s*\)\s*\{', s)
    if m and 'backup_sepolicy NULL -> EAGAIN' not in s:
        dbg = """
        pr_err(\"KernelSU: OP61 selinux_hide DIAG: backup_sepolicy NULL -> EAGAIN\\n\");"""
        s = s[:m.end()] + dbg + s[m.end():]
    elif not m:
        print('OP61: backup_sepolicy NULL guard not found in', p)

    needle = 'ret = ksu_lsm_hook(&selinux_setprocattr_hook);'
    if needle in s and 'lsm_hook returned' not in s:
        s = s.replace(needle, needle + '\n    pr_err("KernelSU: OP61 selinux_hide DIAG: selinux_setprocattr hook returned %d\\n", ret);', 1)

    m = re.search(r'(\b(?:static\s+)?void\s+ksu_selinux_hide_drop_backup_if_unused\s*\([^)]*\)\s*\{)', s)
    if m and 'drop_backup called' not in s:
        dbg = """
    pr_err(\"KernelSU: OP61 selinux_hide DIAG: drop_backup called, backup_sepolicy=%px\\n\", backup_sepolicy);"""
        s = s[:m.end()] + dbg + s[m.end():]
    elif not m:
        print('OP61: drop_backup_if_unused() not found in', p)

    p.write_text(s)
    print('OP61: runtime diagnostics injected into', p)
PY_OP61_DIAG
  done
}
...
ensure_op61_selinux_backup_init    "kernel/runtime/ksud_integration.c"
...
ensure_op61_selinux_backup_init    "drivers/kernelsu/runtime/ksud_integration.c"
