...
"""
    ...
    if 'void ksu_selinux_hide_drop_backup_if_unused(void)' not in s:
        insert = """
void ksu_selinux_hide_drop_backup_if_unused(void)
{
    /* OP61: backup_sepolicy must survive until selinux_hide is initialized. */
    pr_info("KernelSU: OP61 selinux_hide: retaining backup_sepolicy (helper was absent)\\n");
}
"""
        anchor = '#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0)'
        if anchor in s:
            s = s.replace(anchor, insert + '\n' + anchor, 1)
        else:
            s += insert
    print('OP61: drop-backup helper absent; installed inert retention helper in', p)
p.write_text(s)
PY_OP61_BACKUP
}
