...
ensure_sucompat_object_built() {
  ...
  # SukiSU/SUSFS 2.3 callers in fs/{stat,open,exec}.c resolve their
  # ksu_handle_* implementations from feature/sucompat.c. Some SukiSU
  # revisions leave that source file present but omit it from Kbuild after
  # the SUSFS enable patch. That produces the exact final-link failures:
  #   ksu_handle_stat
  #   ksu_handle_faccessat
  #   ksu_handle_post_execveat_sucompat
  # Make sure the object is part of the built-in KSU archive.
  local kbuild="$root/kernel/Kbuild"
  local obj="feature/sucompat.o"
  if [ ! -f "$kbuild" ]; then
    kbuild="$root/drivers/kernelsu/Kbuild"
  fi
  [ -f "$kbuild" ] || return 0
  [ -f "${kbuild%/Kbuild}/feature/sucompat.c" ] || return 0

  if ! grep -Eq '(^|[[:space:]])feature/sucompat\.o([[:space:]]|$)' "$kbuild"; then
    # Prefer an existing feature object list (e.g. obj-y += feature/foo.o).
    if grep -qE '^obj-y[[:space:]]*\+=' "$kbuild"; then
      sed -i '/^obj-y[[:space:]]*+=/a obj-y += feature/sucompat.o' "$kbuild"
    else
      printf '\nobj-y += feature/sucompat.o\n' >> "$kbuild"
    fi
    echo "  [Kbuild] enabled feature/sucompat.o in $kbuild"
  fi
}
