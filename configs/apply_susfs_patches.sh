...
if [ -f drivers/kernelsu/feature/selinux_hide.c ]; then
  if [ "${ANDROID_VER_LOCAL:-}" = "android14" ] && [ "${KERNEL_VER_LOCAL:-}" = "6.1" ]; then
    ...
    if ! grep -qE 'ksu_patch_text[[:space:]]*\(' drivers/kernelsu/feature/selinux_hide.c; then
      echo "::error::OP 6.1 selinux_hide.c lost ksu_patch_text(); runtime SELinux hiding would be incomplete"; exit 1
    fi
    echo "✅ OP 6.1 selinux_hide.c retains ksu_patch_text()"
