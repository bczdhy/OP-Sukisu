...
apply_known_susfs_reject() {
  ...
  # First choice: apply the exact hunk produced by the SUSFS patch command.
  # A .rej file is a valid unified-diff fragment and preserves the upstream
  # SUSFS change instead of reimplementing it in this wrapper.
  ...
  # The adb_root reject is the final SUSFS hunk in this file, and SukiSU may
  # have changed the surrounding function signatures enough that GNU patch
  # cannot match it. Reconcile that exact upstream change structurally: only
  # touch the legacy wrapper forms named by the reject.
...
required = (
    "do_ksu_adb_root_handle_execveat",
...
replacement = "    escape_to_root_for_adb_root();\n\n    ret = escape_with_root_profile();\n    if (ret)\n        pr_err(\"escape_with_root_profile() failed: %d\\n\", (int)ret);\n\n    return 0;"
if needle in s and "ret = escape_with_root_profile();" not in s:
...
old_exec = function_span(s, "long ksu_adb_root_handle_execve(struct pt_regs *regs)")
old_execat = function_span(s, "long ksu_adb_root_handle_execveat(struct pt_regs *regs)")
if not old_exec or not old_execat:
    raise SystemExit(2)
...
PY_ADB_RECONCILE
    then
      echo "  ✅ Reconciled SUSFS adb_root execve→execveat wrapper directly from the reject hunk: $rej"
      rm -f "$rej"
      return 0
    fi
  fi
