...
ensure_backup_sepolicy_symbol() {
  ...
  # selinux_hide.c consumes backup_sepolicy, but SUSFS' enable patch may remove
  # the extern from ksu.h. The actual storage belongs in selinux/rules.c.
  # Do NOT define a second copy inside selinux_hide.c: that creates a separate
  # backup pointer (or a duplicate symbol if rules.c already defines it).
  python3 - "$root" <<'PY_BACKUP_SEPOLICY'
...
targets = [
    root / "kernel/feature/selinux_hide.c",
    root / "drivers/kernelsu/feature/selinux_hide.c",
]
headers = [
    root / "kernel/include/ksu.h",
    root / "drivers/kernelsu/include/ksu.h",
]
rule_files = [
    root / "kernel/selinux/rules.c",
    root / "drivers/kernelsu/selinux/rules.c",
]

targets = [p for p in targets if p.is_file()]
headers = [p for p in headers if p.is_file()]
...
extern_re = re.compile(
    r'(?m)^[ \t]*extern[ \t]+struct[ \t]+selinux_policy[ \t]*\*'
    r'[ \t]*backup_sepolicy[ \t]*;[ \t]*$'
)

# Remove the bad local definition previously injected into selinux_hide.c.
# Replace it with an extern declaration; the storage must be shared with rules.c.
for p in targets:
    s = p.read_text()
    s2, n = re.subn(
        r'(?m)^[ \t]*/\* SUSFS/SukiSU compatibility: selinuxfs expects the policy '
        r'backup symbol\. \*/\n[ \t]*struct[ \t]+selinux_policy[ \t]*\*[ \t]*'
        r'backup_sepolicy[ \t]*;[ \t]*\n?',
        '',
        s,
    )
    # Also remove an un-commented accidental definition if an older run created one.
    if n == 0:
        s2, n = re.subn(
            r'(?m)^[ \t]*struct[ \t]+selinux_policy[ \t]*\*[ \t]*'
            r'backup_sepolicy[ \t]*;[ \t]*\n',
            '',
            s2,
        )
...
# Check whether a real definition already exists anywhere in the relevant KSU tree.
all_defs = []
        ...
        for p in base.rglob("*"):
            if p.suffix in (".c", ".h") and p.is_file():
                try:
                    ...
                    continue
                if definition_re.search(t):
                    all_defs.append(p)

# The normal upstream location is selinux/rules.c. If a skewed tree lost the
# definition entirely, restore exactly one definition there.
if not all_defs and rule_files:
    p = rule_files[0]
    s = p.read_text()
    if not definition_re.search(s):
        marker = '#include "xfrm.h"'
        line = '\n\n/* KernelSU SELinux policy snapshot shared with selinux_hide. */\nstruct selinux_policy *backup_sepolicy;\n'
        if marker in s:
...
# Ensure consumers see the shared definition through ksu.h.
extern_line = 'extern struct selinux_policy *backup_sepolicy;'
for p in headers:
    s = p.read_text()
    if not extern_re.search(s):
        # Put it after the existing ksu_cred declaration when possible.
        m = re.search(r'(?m)^[ \t]*extern[ \t]+struct[ \t]+cred[ \t]*\*[ \t]*ksu_cred[ \t]*;[ \t]*$', s)
        if m:
            s = s[:m.end()] + "\n" + extern_line + s[m.end():]
        else:
            # Keep it inside the header guard, before the final #endif.
            lines = s.splitlines()
            idx = next((i for i in range(len(lines)-1, -1, -1)
                        if lines[i].strip().startswith("#endif")), len(lines))
            lines.insert(idx, extern_line)
            s = "\n".join(lines) + ("\n" if s.endswith("\n") else "")
        p.write_text(s)

# Final fallback: if no ksu.h is present, declare it directly in the consumer.
if not headers:
    for p in targets:
        s = p.read_text()
        if not extern_re.search(s):
...
# Hard validation: exactly one storage definition and at least one declaration.
defs = []
for base in (root / "kernel", root / "drivers/kernelsu"):
    if base.is_dir():
        for p in base.rglob("*"):
...
if len(defs) != 1:
    raise SystemExit(
        "backup_sepolicy validation failed: expected exactly one C definition, "
        f"found {len(defs)}: {defs}"
    )

if headers and not any(extern_re.search(p.read_text()) for p in headers):
    raise SystemExit("backup_sepolicy validation failed: ksu.h has no extern declaration")

print(f"  [SELinux-hide] backup_sepolicy definition: {defs[0]}")
print("  [SELinux-hide] backup_sepolicy extern declaration: restored")
PY_BACKUP_SEPOLICY
}
...
# Final backup_sepolicy validation after all compatibility edits.
# Some SUSFS/SukiSU patch combinations modify ksu.h after the initial repair.
ensure_backup_sepolicy_symbol "$KSU_FOLDER"
ensure_backup_sepolicy_symbol "$COMMON_KERNEL_FOLDER"
