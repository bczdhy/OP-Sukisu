...
s, n1 = re.subn(
    r'static\s+long\s+setup_ld_preload\s*\(\s*struct\s+pt_regs\s*\*\s*regs\s*,\s*unsigned\s+long\s*\*\s*envp_p\s*\)',
    'static long setup_ld_preload(void ***envp_user_ptr)', s, count=1)
if n1 != 1:
...
s, n3 = re.subn(
    ...
    'unsigned long *envp_p = (unsigned long *)envp_user_ptr;', s, count=1)
if n3 != 1:
    raise SystemExit("Could not convert envp_p")
