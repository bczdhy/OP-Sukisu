...
if [ -f kernel/feature/adb_root.c.rej ]; then
  ...
  if grep -qE '^[[:space:]]*void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\(' "$target"; then
    ...
    echo "::error::adb_root.c has neither the newer ksu_handle_vfs_fstat() API nor the legacy hooks expected by the reject"
    echo "::error::Refusing to discard kernel/feature/adb_root.c.rej"
    exit 1
  fi
fi
