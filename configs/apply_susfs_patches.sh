...
"$target" || true
  ...
  # backup_sepolicy is owned by KernelSU's selinux/rules.c. Never create a
  # private static copy here: that pointer remains NULL and selinux_hide_enable()
  # returns -EAGAIN even though the real backup was created successfully.
  if grep -q "backup_sepolicy" "$target"; then
    if grep -qE '^[[:space:]]*static[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy' "$target"; then
      sed -i -E 's/^[[:space:]]*static[[:space:]]+(struct[[:space:]]+selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy[[:space:]]*;)/extern \1/' "$target"
    elif ! grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy' "$target" &&
         ! grep -qE '^[[:space:]]*struct[[:space:]]+selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy[[:space:]]*;' "$target"; then
      sed -i '/^#include/a extern struct selinux_policy *backup_sepolicy;' "$target"
    fi
  fi
...
s = p.read_text()
helper = r"""
static bool ksu_lsm_hook_target_matches(void *current_origin, void *target, const char *target_name)
{
    unsigned long start, current_addr;
    unsigned long target_size = 0, current_size = 0;
    unsigned long target_offset = 0, current_offset = 0;
    char target_sym[KSYM_SYMBOL_LEN];
    char current_sym[KSYM_SYMBOL_LEN];
    char *target_mod = NULL, *current_mod = NULL;
    ...
    /* LTO/ICF/KCFI can leave the hlist with a local alias while kallsyms
     * resolves the public symbol to a different address. First compare the
     * resolved function ranges. */
    start = (unsigned long)target;
    current_addr = (unsigned long)current_origin;
    if (kallsyms_lookup_size_offset(start, &target_size, &target_offset) && target_size &&
        current_addr >= start && current_addr < start + target_size)
        return true;

    /* Range lookup can still fail when the resolver returns an alias. Compare
     * kallsyms names as a final fallback; accept compiler-generated
     * target.llvm.<hash> variants too. */
    if (kallsyms_lookup(start, &target_size, &target_offset, &target_mod, target_sym) &&
        kallsyms_lookup(current_addr, &current_size, &current_offset, &current_mod, current_sym)) {
        if (target_name && target_name[0]) {
            size_t n = strlen(target_name);
            if (!strcmp(current_sym, target_name) ||
                (!strncmp(current_sym, target_name, n) && current_sym[n] == '.'))
                return true;
        }
        if (!strcmp(current_sym, target_sym))
            return true;
    }
...
count=s.count(old)
if count == 0:
    raise SystemExit('OP61: no target comparison in lsm_hook.c')
s=s.replace(old, 'if (ksu_lsm_hook_target_matches(current_origin, target, target_name)) {')
p.write_text(s)
print(f'OP 6.1: robust SELinux LSM target matching applied to {p} ({count} comparisons)')
PY_OP61_LSM
}
...
# SELinux-hide backup policy must resolve to the single global object from
# drivers/kernelsu/selinux/rules.c (or kernel/selinux/rules.c).
SELINUX_RULES_OK=0
for _rules in drivers/kernelsu/selinux/rules.c kernel/selinux/rules.c; do
  if [ -f "$_rules" ] && grep -qE '^[[:space:]]*struct selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy[[:space:]]*;' "$_rules"; then
    SELINUX_RULES_OK=1
    break
  fi
done
if [ "$SELINUX_RULES_OK" != "1" ]; then
  echo "::error::SELinux-hide backup_sepolicy global definition was not found in rules.c"
  exit 1
fi
if grep -RnsE '^[[:space:]]*static[[:space:]]+struct selinux_policy[[:space:]]*\*+[[:space:]]*backup_sepolicy' drivers/kernelsu kernel 2>/dev/null; then
  echo "::error::SELinux-hide has a private static backup_sepolicy definition"
  exit 1
fi
