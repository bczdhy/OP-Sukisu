...
# SukiSU selinux_hide is retained because the current SUSFS-patched tree still
# references its runtime symbols from core/boot and security/selinux.  Provide
# the policy backup storage expected by selinux_hide.c when the older KSU tree
# does not define it itself.
ensure_backup_sepolicy_symbol() {
  local root="$1"
  local target="$root/drivers/kernelsu/feature/selinux_hide.c"
  [ -f "$target" ] || return 0

  if ! grep -qE '^[[:space:]]*(struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*)?backup_sepolicy[[:space:]]*=' "$target"; then
    python3 - "$target" <<'PY_BACKUP_SEPOLICY'
from pathlib import Path
import sys
p=Path(sys.argv[1])
s=p.read_text()
needle='#include "../ksu.h"'
line='\n/* SUSFS/SukiSU compatibility: selinuxfs expects the policy backup symbol. */\nstruct selinux_policy *backup_sepolicy;\n'
if not __import__('re').search(r'(?m)^[ \t]*(?:extern[ \t]+)?struct[ \t]+selinux_policy[ \t]*\*[ \t]*backup_sepolicy[ \t]*(?:=|;)', s):
    if needle in s:
...
p.write_text(s)
PY_BACKUP_SEPOLICY
    echo "  [SELinux-hide] ensured backup_sepolicy definition in: $target"
  fi
}
...
PY_HEADER_40939
  ...
  # The syscall-event bridge still provides the syscall-hook entry points used
  # by syscall_hook_manager.c on this OP13R tree. Keep it compiled; the bridge
  # itself is normalized below for the SUSFS API.
  for kb in "$base/Kbuild" "$base/Makefile"; do
    if [ -f "$kb" ] && grep -qE '^[[:space:]]*kernelsu-objs[[:space:]]*\+=[[:space:]]*hook/syscall_event_bridge\.o[[:space:]]*$' "$kb"; then
      echo "  [syscall bridge] keeping hook/syscall_event_bridge.o in $kb"
    fi
  done
...
# Final OP13R bridge canonicalization.
repair_op13r_syscall_event_bridge() {
  local target="$1"
  [ -f "$target" ] || return 0
  python3 - "$target" <<'PY_REPAIR_BRIDGE'
from pathlib import Path
import re, sys
p = Path(sys.argv[1])
s = p.read_text()
...
def replace_function(src, name, replacement):
    rx = re.compile(r'(?m)^\s*(?:static\s+)?long\s+__nocfi\s+' + re.escape(name) + r'\s*\([^)]*\)\s*\{')
    m = rx.search(src)
    if not m:
        return src, False
    start = m.start(); brace = src.find('{', m.start())
...
execve = """long __nocfi ksu_hook_execve(int orig_nr, const struct pt_regs *regs)
{
    const char __user **filename_user = (const char __user **)&PT_REGS_PARM1(regs);
    long ret;

    /* SUSFS v2.3 handles execve policy in the filename/execveat path. */
    if (current->pid != 1 && is_init(current_cred()))
        ksu_handle_init_mark_tracker(filename_user);
...
"""
setresuid = """long __nocfi ksu_hook_setresuid(int orig_nr, const struct pt_regs *regs)
{
...
"""
s, _ = replace_function(s, 'ksu_hook_execve', execve)
s, _ = replace_function(s, 'ksu_hook_setresuid', setresuid)
p.write_text(s)
PY_REPAIR_BRIDGE
  if grep -qE 'ksu_handle_setresuid\s*\(\s*old_uid\s*,\s*current_uid\(\)\.val\s*\)' "$target"; then
    echo "::error::legacy 2-argument ksu_handle_setresuid call remains in $target"; exit 1
  fi
  grep -qE 'long[[:space:]]+__nocfi[[:space:]]+ksu_hook_execve\s*\(' "$target" || { echo "::error::ksu_hook_execve missing"; exit 1; }
  grep -qE 'long[[:space:]]+__nocfi[[:space:]]+ksu_hook_setresuid\s*\(' "$target" || { echo "::error::ksu_hook_setresuid missing"; exit 1; }
  echo "✅ Final OP13R syscall bridge canonicalization applied: $target"
}

repair_op13r_syscall_event_bridge "kernel/hook/syscall_event_bridge.c"
repair_op13r_syscall_event_bridge "drivers/kernelsu/hook/syscall_event_bridge.c"
fix_sukisu_linker_symbols
