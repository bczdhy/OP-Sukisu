...
ensure_backup_sepolicy_symbol() {
  ...
  # The definition must have external linkage because SELinux/SUSFS code
  # can reference the same policy snapshot from another translation unit.
  if ! grep -qE '^[[:space:]]*(static[[:space:]]+)?struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*backup_sepolicy[[:space:]]*;' "$cfile"; then
    python3 - "$cfile" <<'PY'
...
if not re.search(
    r'^[ \t]*struct[ \t]+selinux_policy[ \t]*\*[ \t]*backup_sepolicy[ \t]*;',
    s, re.M
):
    ...
    p.write_text(s)
PY
    echo "  ✅ Added global backup_sepolicy definition"
  else
    echo "  ✅ backup_sepolicy definition already present"
  fi
...
if [ -f kernel/feature/adb_root.c.rej ]; then
  ...
  if grep -qE 'ksu_handle_vfs_fstat|ksu_execve_hook_ksud|ksu_execveat_hook_ksud|ksu_execve_hook_ksud_common' \
      kernel/feature/adb_root.c.rej; then

    if grep -qE '^[[:space:]]*void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\(' "$target"; then
      echo "  adb_root.c: newer ksu_handle_vfs_fstat API already present; reject is obsolete"
      rm -f kernel/feature/adb_root.c.rej

    elif grep -qE 'ksu_execve_hook_ksud|ksu_execveat_hook_ksud|ksu_execve_hook_ksud_common' "$target"; then
      echo "::error::adb_root.c still contains the legacy SukiSU execve hooks"
      echo "::error::Cannot discard kernel/feature/adb_root.c.rej safely"
      grep -nE 'ksu_execve_hook_ksud|ksu_execveat_hook_ksud|ksu_execve_hook_ksud_common|ksu_handle_vfs_fstat' "$target" || true
      exit 1

    else
      # Neither side of the version-specific hunk exists in this SukiSU tree.
      # Do not invent a fake ksu_handle_vfs_fstat implementation merely to
      # satisfy the SUSFS reject. The hunk has no applicable target here.
      echo "  adb_root.c: neither legacy execve hooks nor vfs_fstat helper exist"
      echo "  adb_root.c: treating version-specific SUSFS reject as obsolete"
      rm -f kernel/feature/adb_root.c.rej
    fi

  else
    echo "::error::Unknown adb_root.c reject content; refusing to discard it"
    echo "::error::Inspect kernel/feature/adb_root.c.rej before continuing"
