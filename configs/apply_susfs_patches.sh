...
fix_sukisu_syscall_event_bridge() {
  local target="$1"
  [ -f "$target" ] || return 0
  echo "Keeping syscall_event_bridge source unchanged: $target"
  # IMPORTANT: do not rewrite this file. The OP13R tree's syscall bridge and
  # syscall_hook_manager are a matched implementation. SUSFS compatibility is
  # handled at the sucompat/execveat API boundary instead.
  return 0
}
...
echo "✅ syscall_event_bridge preserved without source rewriting: $bridge"
  fi
done
