...
ensure_susfs_vfs_sucompat_impl() {
  local root="$1"
  [ -d "$root" ] || return 0
  ...
  if [ -f "$root/drivers/kernelsu/feature/sucompat.c" ]; then
    c="$root/drivers/kernelsu/feature/sucompat.c"
    h="$root/drivers/kernelsu/feature/sucompat.h"
  elif [ -f "$root/kernel/feature/sucompat.c" ]; then
    c="$root/kernel/feature/sucompat.c"
    h="$root/kernel/feature/sucompat.h"
  else
    return 0
  fi

  # SUSFS 2.x for Android 14 / 6.1 replaces the old syscall-table
  # sucompat stat/faccessat handlers with VFS handlers receiving struct
  # filename **.  A partially applied SUSFS patch can leave the old
  # ksu_handle_*_sucompat implementations while removing these two public
  # entry points.  The final OP13R tree is built from drivers/kernelsu, so
  # restore the actual SUSFS implementations there before the final check.
  if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_faccessat[[:space:]]*\(' "$c"; then
    cat >> "$c" <<'VFS_SUCOMPAT_EOF'
...
VFS_SUCOMPAT_EOF
    echo "  [sucompat] restored SUSFS VFS faccessat implementation in $c"
  fi

  if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_stat[[:space:]]*\(' "$c"; then
    cat >> "$c" <<'VFS_STAT_EOF'
...
#endif /* CONFIG_KSU_SUSFS */
VFS_STAT_EOF
    echo "  [sucompat] restored SUSFS VFS stat implementation in $c"
  fi

  # The callers in fs/open.c and fs/stat.c include sucompat.h.  Keep the
  # declaration in the same tree as the implementation and inside the guard.
  if [ -f "$h" ]; then
    python3 - "$h" <<'PY_VFS_HDR'
from pathlib import Path
import sys
p=Path(sys.argv[1])
s=p.read_text()
...
for _root in "$KSU_FOLDER" "$COMMON_KERNEL_FOLDER"; do
  ...
  ensure_susfs_vfs_sucompat_impl "$_root"
done
