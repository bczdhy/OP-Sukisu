...
PY_HEADER_40939
  ...
  # flags arguments. SukiSU 40939 compatibility shims sometimes leave the
  # internal struct user_arg_ptr form in the definition, which conflicts with
  # the SUSFS header declaration. Normalize the definition to the exact ABI.
  if [ -f "$c" ]; then
...
s = p.read_text()
pat = re.compile(
    r'int\s+ksu_handle_execveat_sucompat\s*\(\s*int\s*\*fd\s*,\s*'
    ...
    r'(?:int\s*\*|int\s*\*\s*__never_use_flags)\s*\)'
)
def repl(m):
    return ('int ksu_handle_execveat_sucompat(int *fd, struct filename **filename_ptr,\n'
