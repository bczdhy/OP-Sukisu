...
# Final ksud_integration.c repair.  SUSFS v2.3 moved fstat handling into
# ksu_handle_vfs_fstat().  Do this as a literal post-patch repair in the same
# common-kernel working directory used by the build; do not depend on an
# earlier compatibility function having matched the exact signature.
...
# The old ksud execve path no longer consumes argv_user locally. Remove only
# the stale declaration; leave any real argv_user/argv uses untouched.
s, n_argv = re.subn(
    r'^\s*struct\s+user_arg_ptr\s+argv\s*=\s*\{\s*\.ptr\.native\s*=\s*argv_user\s*\};\s*\n',
    '', s, flags=re.M)

# Find the complete ksu_handle_vfs_fstat() body by brace depth. This avoids
# accidentally placing the declarations in an enclosing/neighboring function.
m = re.search(
    r'\b(?:static\s+)?void\s+ksu_handle_vfs_fstat\s*\(\s*int\s+fd\s*,\s*loff_t\s*\*\s*kstat_size_ptr\s*\)\s*\{',
    s,
)
if not m:
    raise SystemExit('ERROR: ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr) was not found after SUSFS patching')
...
while i < len(s) and depth:
    ...
    i += 1
if depth != 0:
    raise SystemExit('ERROR: unterminated ksu_handle_vfs_fstat() body')

body = s[body_start:i-1]

# SUSFS v2.3 upstream places these immediately at function entry.  The RC
# length adjustment later in this same function requires both variables.
needed = []
if not re.search(r'\bloff_t\s+orig_size\s*=\s*\*kstat_size_ptr\s*;', body):
    needed.append('    loff_t orig_size = *kstat_size_ptr;')
if not re.search(r'\bsize_t\s+extra\s*=\s*0\s*;', body):
    needed.append('    size_t extra = 0;')
if needed:
    body = '\n' + '\n'.join(needed) + body
    s = s[:body_start] + body + s[i-1:]

p.write_text(s)
print(f'ksud_integration.c final repair: argv_removed={n_argv}, fstat_locals={len(needed)}')
PYFINALKSUD

  # Hard validation: fail during patching, not later in clang, if the exact
  # SUSFS v2.3 fstat locals are missing or the stale declaration remains.
  if ! grep -qE 'void[[:space:]]+ksu_handle_vfs_fstat[[:space:]]*\([^)]*loff_t[[:space:]]*\*[[:space:]]*kstat_size_ptr' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::SUSFS v2.3 ksu_handle_vfs_fstat() is missing after patching"
    exit 1
  fi
  if ! grep -qE 'loff_t[[:space:]]+orig_size[[:space:]]*=[[:space:]]*\*kstat_size_ptr[[:space:]]*;' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::ksu_handle_vfs_fstat(): orig_size declaration missing after final repair"
    exit 1
  fi
  if ! grep -qE 'size_t[[:space:]]+extra[[:space:]]*=[[:space:]]*0[[:space:]]*;' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::ksu_handle_vfs_fstat(): extra declaration missing after final repair"
    exit 1
  fi
  if grep -qE '^[[:space:]]*struct[[:space:]]+user_arg_ptr[[:space:]]+argv[[:space:]]*=' drivers/kernelsu/runtime/ksud_integration.c; then
    echo "::error::stale struct user_arg_ptr argv declaration remains in ksud_integration.c"
    exit 1
  fi
  echo "✅ Final ksud_integration.c SUSFS v2.3 scope repair validated"
fi
...
# Remove the stale argv local left behind by the deleted legacy 2-argument call.
fn = re.search(r'\b(?:static\s+)?(?:int|long|void)\s+ksu_handle_execveat_ksud\s*\([^;{]*\)\s*\{', s, re.S)
if fn:
    start = fn.end()
    depth = 1
    i = start
    while i < len(s) and depth:
        ...
        i += 1
    body = s[start:i-1]
    decl = re.compile(r'^\s*struct\s+user_arg_ptr\s+argv\s*=\s*\{\s*\.ptr\.native\s*=\s*argv_user\s*\};\s*$', re.M)
    if decl.search(body) and len(re.findall(r'\bargv\b', body)) == 1:
        body = decl.sub('', body, count=1)
        s = s[:start] + body + s[i-1:]

# Restore the two locals in the SUSFS VFS-fstat helper, if that helper exists.
pat = r'(\b(?:static\s+)?(?:void|int|long)\s+ksu_handle_vfs_fstat\s*\(\s*int\s+fd\s*,\s*loff_t\s*\*\s*kstat_size_ptr\s*\)\s*\{)'
m = re.search(pat, s)
if m:
    body_start = m.end()
    depth = 1
    ...
    if not re.search(r'\bsize_t\s+extra\s*=\s*0\s*;', body):
        prefix += '\n    size_t extra = 0;'
    if prefix:
        s = s[:body_start] + prefix + s[body_start:]
