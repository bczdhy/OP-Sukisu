...
# SELinux-hide is intentionally excluded from this SUSFS build.
# SUSFS provides its own hide mechanisms, while SukiSU's optional selinux_hide
# object is not required for the SUSFS integration used by this workflow.  Do
# not patch its source or try to repair backup_sepolicy; simply keep the source
# tree intact and remove the object from Kbuild so it is not compiled.
disable_sukisu_selinux_hide() {
  local root="$1"
  local kbuild=""
  local changed=0

  if [ -f "$root/kernel/Kbuild" ]; then
    kbuild="$root/kernel/Kbuild"
  elif [ -f "$root/drivers/kernelsu/Kbuild" ]; then
    kbuild="$root/drivers/kernelsu/Kbuild"
  fi

  [ -n "$kbuild" ] || return 0

  if grep -qE '^[[:space:]]*(kernelsu-objs|obj-y)[[:space:]]*\+=[[:space:]]*(feature/)?selinux_hide\.o[[:space:]]*$' "$kbuild"; then
    sed -i -E '/^[[:space:]]*(kernelsu-objs|obj-y)[[:space:]]*\+=[[:space:]]*(feature\/)?selinux_hide\.o[[:space:]]*$/d' "$kbuild"
    changed=1
  fi

  if [ "$changed" -eq 1 ]; then
    echo "  ✅ Disabled SukiSU selinux_hide object in: $kbuild"
  else
    echo "  ℹ️ SukiSU selinux_hide object not present in: $kbuild"
  fi
}
...
disable_sukisu_selinux_hide   "$KSU_FOLDER"
disable_sukisu_selinux_hide   "$COMMON_KERNEL_FOLDER"
