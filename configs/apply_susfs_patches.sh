...
exit 1
  ...
  if grep -q "backup_sepolicy" "$target"; then
    grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]+\*backup_sepolicy[[:space:]]*;' "$target" || {
      echo "::error::SELinux-hide backup_sepolicy is not extern in $target; refusing to build a broken private copy"
      exit 1
    }
  fi
...
"$target" || true
  ...
  # SukiSU's backup policy is owned by selinux/rules.c and must be shared with
  # selinux_hide.c.  A private/static copy here is a critical bug: the hide
  # feature then sees its own NULL backup_sepolicy and returns -EAGAIN when
  # the manager enables "Hide SELinux modifications".
  if grep -q "backup_sepolicy" "$target"; then
    # Remove any stale local definition inserted by older versions of this
    # build script, then bind selinux_hide.c to the real global definition.
    sed -i '/^[[:space:]]*static[[:space:]]\+struct[[:space:]]\+selinux_policy[[:space:]]\+\*backup_sepolicy[[:space:]]*;/d' "$target" || true
    if ! grep -qE '^[[:space:]]*extern[[:space:]]+struct[[:space:]]+selinux_policy[[:space:]]+\*backup_sepolicy[[:space:]]*;' "$target"; then
      awk '
        /^#include/ { print; last_include = NR; next }
        last_include && !inserted {
          print ""
          print "extern struct selinux_policy *backup_sepolicy;"
          print ""
          inserted = 1
        }
        { print }
