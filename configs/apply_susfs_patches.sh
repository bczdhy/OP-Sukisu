...
if [ -f "$target" ]; then
  ...
  # IMPORTANT: Keep the SukiSU SELinux text-patching calls intact.
  # These calls are part of the runtime selinux_hide implementation.  The
  # previous compatibility cleanup replaced ksu_patch_text() with ret = 0
  # and removed the calls entirely, which allowed the kernel to build but
  # prevented the SELinux-hide hooks from being installed at runtime.
  for target in \
"$KSU_FOLDER/kernel/feature/selinux_hide.c" \
"$COMMON_KERNEL_FOLDER/drivers/kernelsu/feature/selinux_hide.c"; do
if [ -f "$target" ]; then
  echo "  Preserving SukiSU SELinux hide patching in $target"
fi
  done
...
# SELinux-hide is not an optional/expected reject: if the SUSFS patch could
# not modify selinux_hide.c, do not silently ship a kernel with a non-working
# Manager toggle.
if [ -f "kernel/feature/selinux_hide.c.rej" ]; then
  echo "::error::SUSFS SELinux-hide patch rejected in kernel/feature/selinux_hide.c"
  cat "kernel/feature/selinux_hide.c.rej"
  exit 1
fi
...
if [ -f drivers/kernelsu/feature/selinux_hide.c ]; then
  # selinux_hide.c must retain its runtime patching machinery.  Do not reject
  # ksu_patch_text() itself; its presence is required for SELinux hiding.
  if ! grep -qE 'ksu_patch_text[[:space:]]*\(' drivers/kernelsu/feature/selinux_hide.c; then
    echo "::error::SELinux hide implementation has no ksu_patch_text() call; runtime hooks were likely stripped"
    exit 1
  fi
