...
# The SUSFS reject changes these exact two wrapper layers.  Require the old
# symbols before touching anything, so this path cannot silently modify an
# unrelated adb_root.c.
old_do = "static long do_ksu_adb_root_handle_execve(const char __user *filename_user, struct pt_regs *regs, unsigned long *envp_p)"
old_exec = "long ksu_adb_root_handle_execve(struct pt_regs *regs)"
old_execat = "long ksu_adb_root_handle_execveat(struct pt_regs *regs)"

if old_do not in s or old_exec not in s or old_execat not in s:
    raise SystemExit(2)

# 1. Convert the private helper to the SUSFS execveat form.
s = s.replace(old_do,
    "static long do_ksu_adb_root_handle_execveat(const char *filename, void ***envp_user_ptr)", 1)
s = s.replace("is_exec_adbd(filename_user)", "is_exec_adbd(filename)", 1)
s = s.replace("setup_ld_preload(regs, envp_p)", "setup_ld_preload(envp_user_ptr)", 1)

# The reject explicitly adds escape_with_root_profile() after the adb-root
# escape.  Add it only if that exact call is not already present.
...
if "ret = escape_with_root_profile();" not in s:
    if marker not in s:
        raise SystemExit(2)
    s = s.replace(marker, profile, 1)

# 2. Remove the obsolete execve wrapper completely.
span = function_span(s, old_exec)
if not span:
    raise SystemExit(2)
a, b = span
s = s[:a] + s[b:]

# 3. Replace the execveat wrapper with the exact SUSFS API/body.
span = function_span(s, old_execat)
if not span:
    raise SystemExit(2)
a, b = span
...
# 4. The official SUSFS patch also changes the public header.  The main patch
# may already have done this; otherwise make only that exact declaration
# change when the old declaration is present.
h = p.parent.parent / "kernel" / "feature" / "adb_root.h"
# apply_known_susfs_reject runs from the kernel root, so the header is resolved
# relative to cwd rather than the workflow script location.
header = Path("kernel/feature/adb_root.h")
if header.is_file():
    hs = header.read_text()
    old_decl = "long ksu_adb_root_handle_execve(struct pt_regs *regs);\nlong ksu_adb_root_handle_execveat(struct pt_regs *regs);"
    new_decl = "long ksu_adb_root_handle_execveat(const char *filename, void __user ***envp_user_ptr);"
    if old_decl in hs:
...
PY_ADB_RECONCILE
    then
      echo "  ✅ Reconciled SUSFS adb_root execve→execveat transformation directly from the reject hunk: $rej"
      rm -f "$rej"
      return 0
    fi
  fi
