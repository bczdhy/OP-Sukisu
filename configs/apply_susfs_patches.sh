...
ensure_sucompat_object_built() {
  ...
  # The KSU aggregate object is normally named kernelsu.o and its object list
  # is kernelsu-objs. Adding feature/sucompat.o to obj-y is ineffective here:
  # the final drivers/kernelsu/built-in.a is assembled from kernelsu-objs.
  # That was the reason the previous repair still linked without sucompat.o.
  local kbuild=""
  for candidate in "$root/kernel/Kbuild" "$root/kernel/Makefile" "$root/drivers/kernelsu/Kbuild" "$root/drivers/kernelsu/Makefile"; do
    if [ -f "$candidate" ]; then
      kbuild="$candidate"
      break
    fi
  ...
  # Prefer the aggregate object variable used by the KSU Kbuild:
  #   kernelsu-objs := ...
  #   obj-$(CONFIG_KSU) += kernelsu.o
  # If a fork uses another aggregate name, derive it from obj-$(CONFIG_KSU).
  local aggregate=""
  ...
  if ! grep -Fqx "$object_line" "$kbuild"; then
    # Remove any previous incorrect obj-y repair for this object, then add the
    # object to the aggregate that actually produces drivers/kernelsu/built-in.a.
...
for p in (root/'kernel/runtime/ksud_integration.c', root/'drivers/kernelsu/runtime/ksud_integration.c'):
    ...
    for name in ('orig_sys_read','orig_sys_fstat'):
        ...
        if len(re.findall(r'\b'+name+r'\b', s)) == 1:
    ...
    for _sym in ksu_handle_post_execveat_sucompat ksu_handle_stat ksu_handle_faccessat; do
      if grep -qE "^[[:space:]]*(int|long)[[:space:]]+${_sym}[[:space:]]*\(" "$_root/fs/exec.c" "$_root/fs/stat.c" "$_root/fs/open.c" 2>/dev/null; then
        if ! grep -qE "^[[:space:]]*(int|long)[[:space:]]+${_sym}[[:space:]]*\(" "$_sc"; then
          echo "::error::${_sym} is called by the kernel but has no implementation in $_sc"
          exit 1
        fi
      fi
    done
