...
ensure_op61_selinux_hide_init() {
  local target="$1"
  [ -f "$target" ] || return 0
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then return 0; fi
  echo "Ensuring OP 6.1 SukiSU SELinux-hide initialization in: $target"
  local root_dir; root_dir="$(dirname "$(dirname "$target")")"
  grep -q '#include "hook/lsm_hook.h"' "$target" || sed -i '/#include "ksu.h"/a #include "hook/lsm_hook.h"' "$target"
  grep -q '#include "feature/selinux_hide.h"' "$target" || sed -i '/#include "ksu.h"/a #include "feature/selinux_hide.h"' "$target"
  if ! grep -RqsE '\bksu_lsm_hook_init[[:space:]]*\(' "$root_dir" --include='*.c' 2>/dev/null; then echo "::error::ksu_lsm_hook_init() is not defined in the OP 6.1 KSU tree"; exit 1; fi
  if ! grep -RqsE '\bksu_selinux_hide_init[[:space:]]*\(' "$root_dir" --include='*.c' 2>/dev/null; then echo "::error::ksu_selinux_hide_init() is not defined in the OP 6.1 KSU tree"; exit 1; fi
  if ! grep -qE '^[[:space:]]*ksu_lsm_hook_init[[:space:]]*\(\);' "$target"; then
    if grep -qE '^[[:space:]]*ksu_adb_root_init[[:space:]]*\(\);' "$target"; then sed -i '/^[[:space:]]*ksu_adb_root_init[[:space:]]*();/a\    ksu_lsm_hook_init();' "$target"
    elif grep -qE '^[[:space:]]*ksu_feature_init[[:space:]]*\(\);' "$target"; then sed -i '/^[[:space:]]*ksu_feature_init[[:space:]]*();/a\    ksu_lsm_hook_init();' "$target"
    else echo "::error::No safe init anchor for ksu_lsm_hook_init() in $target"; exit 1; fi
  fi
  if ! grep -qE '^[[:space:]]*ksu_selinux_hide_init[[:space:]]*\(\);' "$target"; then sed -i '/^[[:space:]]*ksu_lsm_hook_init[[:space:]]*();/a\    ksu_selinux_hide_init();' "$target"; fi
  if grep -RqsE '\bksu_lsm_hook_exit[[:space:]]*\(' "$root_dir" --include='*.c' 2>/dev/null && ! grep -qE '^[[:space:]]*ksu_lsm_hook_exit[[:space:]]*\(\);' "$target"; then
    if grep -qE '^[[:space:]]*ksu_selinux_hide_exit[[:space:]]*\(\);' "$target"; then sed -i '/^[[:space:]]*ksu_selinux_hide_exit[[:space:]]*();/a\    ksu_lsm_hook_exit();' "$target"
    elif grep -qE '^[[:space:]]*ksu_feature_exit[[:space:]]*\(\);' "$target"; then sed -i '/^[[:space:]]*ksu_feature_exit[[:space:]]*();/i\    ksu_lsm_hook_exit();' "$target"; fi
  fi
  grep -qE '^[[:space:]]*ksu_lsm_hook_init[[:space:]]*\(\);' "$target" || { echo "::error::OP 6.1 missing ksu_lsm_hook_init()"; exit 1; }
  grep -qE '^[[:space:]]*ksu_selinux_hide_init[[:space:]]*\(\);' "$target" || { echo "::error::OP 6.1 missing ksu_selinux_hide_init()"; exit 1; }
  echo "✅ OP 6.1 SELinux-hide init/exit wiring ensured"
}
...
fix_sukisu_lsm_hook_c_op61() {
  local target="$1"
  [ -f "$target" ] || return 0

  # OP Android 14 / kernel 6.1: KCFI+LTO can register
  # selinux_setprocattr through an LTO-local alias. SukiSU's generic
  # exact-pointer hlist match can therefore return -ENOENT.
  if [ "${ANDROID_VER_LOCAL:-}" != "android14" ] || [ "${KERNEL_VER_LOCAL:-}" != "6.1" ]; then
    return 0
  fi

  echo "Applying OP 6.1 SELinux LSM-hook compatibility fix: $target"

  if ! grep -q 'ksu_op61_lsm_target_matches' "$target"; then
    python3 - "$target" "" <<'PY'
...
p = Path(sys.argv[1])
s = p.read_text()
helper = base64.b64decode(sys.argv[2]).decode()
needle = "int ksu_lsm_hook(struct ksu_lsm_hook *hook)"
if needle not in s:
    raise SystemExit("ksu_lsm_hook() anchor not found")
s = s.replace(needle, helper + needle, 1)
old = "if (current_origin == target) {"
new = "if (ksu_op61_lsm_target_matches(target, current_origin, target_name)) {"
if s.count(old) != 1:
    raise SystemExit(f"Expected one hlist target comparison, found {s.count(old)}")
...
PY
  ...
  if ! grep -q 'ksu_op61_lsm_target_matches' "$target"; then
    echo "::error::OP 6.1 SELinux LSM-hook compatibility fix was not inserted into $target"
    exit 1
  fi

  echo "✅ OP 6.1 SELinux LSM-hook compatibility fix applied"
}
...
ensure_op61_selinux_hide_init    "kernel/core/init.c"
...
ensure_op61_selinux_hide_init    "drivers/kernelsu/core/init.c"
...
fix_sukisu_lsm_hook_c_op61      "drivers/kernelsu/hook/lsm_hook.c"
...
if [ -f drivers/kernelsu/feature/selinux_hide.c ]; then
  if [ "${ANDROID_VER_LOCAL:-}" = "android14" ] && [ "${KERNEL_VER_LOCAL:-}" = "6.1" ]; then
    if grep -nE 'ksu_late_loaded' drivers/kernelsu/feature/selinux_hide.c; then
      echo "::error::ksu_late_loaded still exists in OP 6.1 selinux_hide.c"; exit 1
    fi
    # Do not require a literal ksu_patch_text() call here. SukiSU/SUSFS
    # revisions can implement the 6.1 SELinux hiding path without that exact
    # call site. The previous check caused a false build failure even when
    # the compatibility patch itself had completed successfully.
    echo "ℹ️ OP 6.1 selinux_hide.c: skipping literal ksu_patch_text() check"
  elif grep -nE 'ksu_late_loaded|ksu_patch_text[[:space:]]*\(' drivers/kernelsu/feature/selinux_hide.c; then
    echo "::error::Unsupported SukiSU/SUSFS call still exists in drivers/kernelsu/feature/selinux_hide.c"; exit 1
  fi
