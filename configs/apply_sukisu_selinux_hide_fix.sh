...
fix_selinux_hide_api() {
  ...
  # Some SukiSU SELinux-hide revisions reference backup_sepolicy but rely on
  # an older declaration that is absent from newer OnePlus kernel trees.
  # Detect the actual declaration, not merely the symbol name, and restore it
  # when the implementation uses backup_sepolicy.
  if grep -q 'backup_sepolicy' "$target" && ! grep -Eq '^[[:space:]]*(static[[:space:]]+)?struct[[:space:]]+selinux_policy[[:space:]]*\*[[:space:]]*backup_sepolicy[[:space:]]*;' "$target"; then
    awk '
      ...
      { print }
      END { if (!inserted) { print ""; print "static struct selinux_policy *backup_sepolicy;" } }
    ' "$target" > "$target.tmp"
    mv "$target.tmp" "$target"
    echo "Restored missing backup_sepolicy declaration in: $target"
  fi
