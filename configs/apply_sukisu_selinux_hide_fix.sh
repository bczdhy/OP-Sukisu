...
echo "::group::Apply SukiSU SELinux-hide fix (independent of SUSFS)"
...
verify_selinux_hide_hook() {
  local root="$1"
  local ok=0
  for f in "$root/kernel/hook/lsm_hook.c" "$root/hook/lsm_hook.c"; do
    [ -f "$f" ] && grep -q "selinux_setprocattr" "$f" && ok=1
  done
  for f in "$root/kernel/Kbuild" "$root/Kbuild"; do
    [ -f "$f" ] && grep -q "hook/lsm_hook.o" "$f" && ok=1
  done
  if [ "$ok" -eq 0 ]; then
    echo "::error::SELinux-hide hook verification failed: lsm_hook integration not found"
    exit 1
  fi
  echo "✅ SELinux-hide hook integration verified"
}

verify_selinux_hide_hook "$KSU_FOLDER"
...
# OnePlus common-tree mirror, when present.
